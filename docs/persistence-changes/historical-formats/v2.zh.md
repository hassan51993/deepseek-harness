---
description: "كامل Session V2 حفظ دائم نوع، مصدر دليل و شيء إدارة تحرير رمز مشاركة اعتبار."
kind: persistence-format
---

# Session V2 حفظ دائم صيغة

[English](v2.md) | العربية

## عام وصف

فحص قراءة `dsh-v0.1.3-alpha.2` مقابل V2 حفظ دائم نوع.Assistant تدفق داخل تضمين في message أو attempt حدث في، شيء إدارة header اشتراط `isSeeded`، لم يعد تخزين `seedLength`. هذا لقطة إبقاء V3 نظام رسالة و معلومة غلاف تغيير قبل V2 إعلان.

## دليل

- [مصدر دليل](#evidence)
- [صيغة خاص سمة](#characteristics)
- [آلة جهاز إعلان](#declaration)
- [كامل schema](#schema)
- [تحقق و حد](#verification)
...
مصدر لـ قد إصدار tag `dsh-v0.1.3-alpha.2`، ذلك كتابة جهاز معتاد كمية لـ 2.[tag سجل](../releases/dsh-v0.1.3-alpha.2.zh.md) شرح متبادل مجاور إصدار بين تغير. هذا كامل دليل معا يتضمن لم تغير أصل نوع و هذا tag تغير أصل نوع.

رفع أخذ إعلان قدوم ذاتي `packages/core/session/src/types.ts`،`packages/session/session-persistence-jsonl/src/format.ts`، و كل رقم واحد جهة `SessionEventMap` إعلان دمج و ذلك يمكن بلوغ نوع. تحت جهة شفرة المصدر موضع إشارة نحو هذا تاريخ شفرة المصدر شجرة، بينما غير حالي ملف.

<a id="characteristics"></a>
## صيغة خاص سمة

التالي واقع وصف الذي اختيار فحص نقطة. تحت جهة توليد جدول إطار إبقاء دقيق حقل نوع و اختياري صفة.

- `SessionHeader.version` تحليل لـ حرف وجه كمية 2. منطق و شيء إدارة header متساو اشتراط `isSeeded`؛ شيء إدارة header لا يوجد `seedLength`.
- لا وجود `assistant/chunk`.`assistant/message.data` و فقط سجل استخدام طريق `assistant/attempt.data` كل اشتراط داخل تضمين `stream`.
- surface استبدال إعلان إبقاء `start` / `end`؛ توجيه النظام نص ما زال يقع في `request/header.data.header.system`. لا وجود V3 `system/message` حدث.

[V2 codec](../../../packages/session/session-format-v1-to-v2/src/codec.ts) مسؤول كل سطر واحد حدث تحرير رمز، نطاق تحرير رمز مصدر مرجع، و من `session/end-seed` علامة دفع توجيه وراثة قطع نقطة.[V2 إلى V3 مواصفة](../../../packages/session/session-format-v2-to-v3/README.zh.md#v2-to-v3-specification) مسؤول نظام رسالة إدراج دخول،PTC حدث اسم و مواصفة معلومة غلاف تحويل.

مقابل في قد وراثة V2 Session، الأكثر بعد واحد يحمل `data.inherited: true` `session/end-seed` حدث بـ ذلك ترتيب رقم موضع معرف وراثة قطع نقطة. شيء إدارة `sourceEventSeqs` يمكن استخدام نطاق تحرير رمز؛ منطق schema إبقاء توسيع بعد عدد قيمة عدد مجموعة.

<a id="declaration"></a>
## آلة جهاز إعلان

[Schema دليل](v2.schema.json).
...
[كامل آلة جهاز دليل](v2.schema.json) يتضمن 56 عدد أصل نوع و 435 نوع يمكن بلوغ نوع. ملخص يتضمن كل مرجع حقل؛ شفرة المصدر اسم و مسار وصف الذي اختيار تاريخ شفرة المصدر شجرة.

### حفظ دائم نوع إشارة نقش

| أصل نوع | صنف آخر | SHA-256 | قد تحليل نوع |
|---|---|---|---|
...
<details>
<summary>كامل تحليل نوع</summary>

### قد تحليل حفظ دائم نوع

كل نوع تعريف فقط صف خروج مرة. مرجع إبقاء مشترك و تمرير عودة علاقة؛ تعريف جانب ملخص شمول غطاء ذلك كامل يمكن بلوغ بنية. شفرة المصدر اسم و موضع معرف إعلان مصدر، لكن لا مشاركة و ملخص حساب حساب.
...
مصدر:`packages/core/session/src/types.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:agent-preset/selected[0].data`](#persistence-type-eventagent-presetselected0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"agent-preset/selected"` |
...
مصدر:`packages/preset/agent-presets/src/session.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `agentPreset` | مطلوب | `string` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:agent/inbox/spliced[0].data`](#persistence-type-eventagentinboxspliced0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"agent/inbox/spliced"` |
...
مصدر:`packages/core/agent/src/types.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `inserted` | مطلوب | [`event:agent/inbox/spliced[0].data.inserted`](#persistence-type-eventagentinboxspliced0datainserted) |
| `outcome` | اختياري | `"canceled"` |
| `removedCount` | اختياري | `number` |
| `start` | مطلوب | `number` |
| `target` | مطلوب | [`packages/core/agent/src/types.ts#InboxTarget`](#persistence-type-packagescoreagentsrctypestsinboxtarget) |
...
[`packages/llm/llm/src/message.ts#UserMessage`](#persistence-type-packagesllmllmsrcmessagetsusermessage) عدد مجموعة.
...
[`packages/llm/llm/src/types.ts#ContentBlock`](#persistence-type-packagesllmllmsrctypestscontentblock) عدد مجموعة.
...
مصدر:`packages/attachment/attachment/src/types.ts`
...
مصدر:`packages/core/session/src/types.ts` · `packages/interaction/commands/src/types.ts` · `packages/llm/llm/src/message.ts` · `packages/session/session-title/src/types.ts`
...
مصدر:`packages/api/session-controller/src/types.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `clientTimeZone` | اختياري | `string` |
| `kind` | مطلوب | `"user"` |
| `rpcId` | مطلوب | `string` |
...
[`event:agent/inbox/spliced[0].data.inserted[0].source[15].references[0]`](#persistence-type-eventagentinboxspliced0datainserted0source15references0) عدد مجموعة.
...
مصدر:`packages/context/session-reference/src/types.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `capturedFormatVersion` | اختياري | `number` |
| `capturedThroughSeq` | مطلوب | [`event:compaction/end[0].data.turn`](#persistence-type-eventcompactionend0dataturn) |
| `compacted` | مطلوب | `boolean` |
| `inputIndex` | مطلوب | `number` |
| `label` | مطلوب | `string` |
| `omittedBytes` | مطلوب | `number` |
| `omittedMessages` | مطلوب | `number` |
| `originalMessages` | مطلوب | `number` |
| `retainedMessages` | مطلوب | `number` |
| `sessionId` | مطلوب | `string` |
| `truncated` | مطلوب | `boolean` |
...
[`event:agent/inbox/spliced[0].data.inserted[0].source[16].entries[0]`](#persistence-type-eventagentinboxspliced0datainserted0source16entries0) عدد مجموعة.
...
مصدر:`packages/skill/tool-skill/src/index.ts`
...
[`packages/context/agent-instructions/src/render.ts#AgentInstructionChange`](#persistence-type-packagescontextagent-instructionssrcrendertsagentinstructionchange) عدد مجموعة.
...
مصدر:`packages/webhook/webhook/src/types.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `deliveryId` | مطلوب | `string` |
| `form` | مطلوب | `"notice"` |
| `kind` | مطلوب | `"webhook"` |
| `provider` | مطلوب | `string` |
| `ruleId` | مطلوب | `string` |
| `source` | مطلوب | `string` |
| `summary` | مطلوب | `string` |
...
| `kind` | مطلوب | `"plugin"` |
| `plugin` | مطلوب | `string` |
| `sections` | مطلوب | [`event:agent/inbox/spliced[0].data.inserted[0].source[4].sections`](#persistence-type-eventagentinboxspliced0datainserted0source4sections) |
...
[`packages/llm/llm/src/message.ts#ContextSnapshotSection`](#persistence-type-packagesllmllmsrcmessagetscontextsnapshotsection) عدد مجموعة.
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:approval/asked[0].data`](#persistence-type-eventapprovalasked0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"approval/asked"` |
...
مصدر:`packages/interaction/user-approval/src/types.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `callId` | اختياري | `string` |
| `id` | مطلوب | `string` |
| `reason` | اختياري | `string` |
| `toolName` | مطلوب | `string` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:approval/decided[0].data`](#persistence-type-eventapprovaldecided0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"approval/decided"` |
...
|---|---|---|
| `id` | مطلوب | `string` |
| `outcome` | مطلوب | [`packages/interaction/user-approval/src/types.ts#ApprovalOutcome`](#persistence-type-packagesinteractionuser-approvalsrctypestsapprovaloutcome) |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:approval/policy[0].data`](#persistence-type-eventapprovalpolicy0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"approval/policy"` |
...
مصدر:`packages/interaction/user-approval/src/index.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `policy` | مطلوب | [`packages/interaction/user-approval/src/index.ts#ApprovalPolicy`](#persistence-type-packagesinteractionuser-approvalsrcindextsapprovalpolicy) |
| `source` | اختياري | `"delegation"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:assistant/attempt[0].data`](#persistence-type-eventassistantattempt0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"assistant/attempt"` |
...
|---|---|---|
| `step` | مطلوب | `number` |
| `stream` | مطلوب | [`event:assistant/attempt[0].data.stream`](#persistence-type-eventassistantattempt0datastream) |
| `turn` | مطلوب | `number` |
...
[`packages/llm/llm/src/assistant-stream.ts#AssistantStreamRecord`](#persistence-type-packagesllmllmsrcassistant-streamtsassistantstreamrecord) عدد مجموعة.
...
مصدر:`packages/llm/llm/src/assistant-stream.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `dt` | مطلوب | [`SessionEventEnvelope[48].sourceEventSeqs`](#persistence-type-sessioneventenvelope48sourceeventseqs) |
| `index` | مطلوب | `number` |
| `texts` | مطلوب | [`event:assistant/attempt[0].data.stream[0][0].texts`](#persistence-type-eventassistantattempt0datastream00texts) |
| `time0` | مطلوب | `number` |
| `type` | مطلوب | `"text-chunks"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `args` | مطلوب | [`event:assistant/attempt[0].data.stream[0][0].texts`](#persistence-type-eventassistantattempt0datastream00texts) |
| `dt` | مطلوب | [`SessionEventEnvelope[48].sourceEventSeqs`](#persistence-type-sessioneventenvelope48sourceeventseqs) |
| `id` | مطلوب | `string` |
| `index` | مطلوب | `number` |
| `name` | اختياري | `string` |
| `time0` | مطلوب | `number` |
| `type` | مطلوب | `"tool-call-chunks"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `chunk` | مطلوب | [`packages/llm/llm/src/types.ts#StreamChunk`](#persistence-type-packagesllmllmsrctypestsstreamchunk) |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"chunk"` |
...
مصدر:`packages/llm/llm/src/types.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `blockType` | مطلوب | [`packages/llm/llm/src/types.ts#ContentBlockType`](#persistence-type-packagesllmllmsrctypestscontentblocktype) |
| `index` | مطلوب | `number` |
| `type` | مطلوب | `"block-start"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `argumentsDelta` | مطلوب | `string` |
| `id` | مطلوب | `string` |
| `index` | مطلوب | `number` |
| `name` | اختياري | `string` |
| `type` | مطلوب | `"tool-call-delta"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `block` | مطلوب | [`packages/llm/llm/src/types.ts#ContentBlock`](#persistence-type-packagesllmllmsrctypestscontentblock) |
| `index` | مطلوب | `number` |
| `type` | مطلوب | `"block-end"` |
...
|---|---|---|
| `type` | مطلوب | `"usage"` |
| `usage` | مطلوب | [`packages/llm/llm/src/types.ts#TokenUsage`](#persistence-type-packagesllmllmsrctypeststokenusage) |
...
| خاصية | وجود | نوع |
|---|---|---|
| `reason` | مطلوب | [`packages/llm/llm/src/types.ts#FinishReason`](#persistence-type-packagesllmllmsrctypestsfinishreason) |
| `replayState` | اختياري | [`packages/llm/llm/src/types.ts#ReplayEnvelope`](#persistence-type-packagesllmllmsrctypestsreplayenvelope) |
| `type` | مطلوب | `"finish"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `failure` | مطلوب | [`packages/llm/llm/src/types.ts#LlmFailure`](#persistence-type-packagesllmllmsrctypestsllmfailure) |
| `kind` | مطلوب | `"aborted"` |
...
مصدر:`packages/core/session/src/types.ts` · `packages/llm/llm/src/types.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `kind` | مطلوب | `"max-tokens"` |
...
`unknown`(لا نفاذ واضح) عدد مجموعة.
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:assistant/message.data`](#persistence-type-eventassistantmessagedata) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `sourceEventSeqs` | اختياري | [`SessionEventEnvelope[48].sourceEventSeqs`](#persistence-type-sessioneventenvelope48sourceeventseqs) |
| `surfaceOp` | اختياري | [`SessionEventEnvelope[48].surfaceOp`](#persistence-type-sessioneventenvelope48surfaceop) |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"assistant/message"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `interrupted` | اختياري | `true` |
| `message` | مطلوب | [`packages/llm/llm/src/message.ts#AssistantMessage`](#persistence-type-packagesllmllmsrcmessagetsassistantmessage) |
| `step` | مطلوب | `number` |
| `stream` | مطلوب | [`event:assistant/attempt[0].data.stream`](#persistence-type-eventassistantattempt0datastream) |
| `turn` | مطلوب | `number` |
| `usage` | اختياري | [`packages/llm/llm/src/types.ts#TokenUsage`](#persistence-type-packagesllmllmsrctypeststokenusage) |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:command/done[0].data`](#persistence-type-eventcommanddone0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"command/done"` |
...
مصدر:`packages/interaction/commands/src/types.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `commandId` | مطلوب | `string` |
| `kind` | مطلوب | [`event:command/done[0].data.kind`](#persistence-type-eventcommanddone0datakind) |
| `sourceEventSeq` | اختياري | `number` |
| `text` | اختياري | `string` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:command/run[0].data`](#persistence-type-eventcommandrun0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"command/run"` |
...
| `commandId` | مطلوب | `string` |
| `name` | مطلوب | `string` |
| `source` | مطلوب | [`event:agent/inbox/spliced[0].data.inserted[0].source[0]`](#persistence-type-eventagentinboxspliced0datainserted0source0) |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:compaction/end[0].data`](#persistence-type-eventcompactionend0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"compaction/end"` |
...
مصدر:`packages/compaction/compaction/src/types.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `compactionId` | مطلوب | `string` |
| `error` | اختياري | `string` |
| `sourceCommandId` | اختياري | `string` |
| `turn` | مطلوب | [`event:compaction/end[0].data.turn`](#persistence-type-eventcompactionend0dataturn) |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:compaction/prune[0].data`](#persistence-type-eventcompactionprune0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"compaction/prune"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `shadowedRange` | مطلوب | [`event:compaction/prune[0].data.shadowedRange`](#persistence-type-eventcompactionprune0datashadowedrange) |
| `shadowedSeqs` | مطلوب | [`SessionEventEnvelope[48].sourceEventSeqs`](#persistence-type-sessioneventenvelope48sourceeventseqs) |
| `shadowedTokenCount` | مطلوب | `number` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:compaction/start[0].data`](#persistence-type-eventcompactionstart0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"compaction/start"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:compaction/summary[0].data`](#persistence-type-eventcompactionsummary0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"compaction/summary"` |
...
|---|---|---|
| `compactionId` | مطلوب | `string` |
| `llmStreamCall` | مطلوب | `true` |
| `maxTokens` | اختياري | `number` |
| `model` | مطلوب | `string` |
| `provider` | مطلوب | `string` |
| `rawOutput` | مطلوب | [`event:agent/inbox/spliced[0].data.inserted[0].content`](#persistence-type-eventagentinboxspliced0datainserted0content) |
| `shadowedRange` | مطلوب | [`event:compaction/prune[0].data.shadowedRange`](#persistence-type-eventcompactionprune0datashadowedrange) |
| `shadowedSeqs` | مطلوب | [`SessionEventEnvelope[48].sourceEventSeqs`](#persistence-type-sessioneventenvelope48sourceeventseqs) |
| `shadowedTokenCount` | مطلوب | `number` |
| `sourceCommandId` | اختياري | `string` |
| `summary` | مطلوب | [`event:agent/inbox/spliced[0].data.inserted[0].content`](#persistence-type-eventagentinboxspliced0datainserted0content) |
| `usage` | اختياري | [`packages/llm/llm/src/types.ts#TokenUsage`](#persistence-type-packagesllmllmsrctypeststokenusage) |
...
| `model` | مطلوب | `string` |
| `provider` | مطلوب | `string` |
| `rawOutput` | اختياري | [`event:agent/inbox/spliced[0].data.inserted[0].content`](#persistence-type-eventagentinboxspliced0datainserted0content) |
| `shadowedRange` | مطلوب | [`event:compaction/prune[0].data.shadowedRange`](#persistence-type-eventcompactionprune0datashadowedrange) |
| `shadowedSeqs` | مطلوب | [`SessionEventEnvelope[48].sourceEventSeqs`](#persistence-type-sessioneventenvelope48sourceeventseqs) |
| `shadowedTokenCount` | مطلوب | `number` |
| `sourceCommandId` | اختياري | `string` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`packages/feedback/message-feedback/src/types.ts#MessageFeedbackDelete`](#persistence-type-packagesfeedbackmessage-feedbacksrctypestsmessagefeedbackdelete) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"feedback/message-delete"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`packages/feedback/message-feedback/src/types.ts#MessageFeedbackPut`](#persistence-type-packagesfeedbackmessage-feedbacksrctypestsmessagefeedbackput) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"feedback/message-put"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:feedback/record[0].data`](#persistence-type-eventfeedbackrecord0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"feedback/record"` |
...
مصدر:`packages/feedback/command-feedback/src/index.ts`
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`packages/goal/goal/src/domain.ts#GoalChangeMeta`](#persistence-type-packagesgoalgoalsrcdomaintsgoalchangemeta) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"goal/change"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:hook/invoked[0].data`](#persistence-type-eventhookinvoked0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"hook/invoked"` |
...
مصدر:`packages/hooks/hook-protocol/src/types.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `dialect` | مطلوب | [`packages/hooks/hook-protocol/src/types.ts#HookDialect`](#persistence-type-packageshookshook-protocolsrctypestshookdialect) |
| `handlerId` | مطلوب | `string` |
| `matcher` | اختياري | `string` |
| `point` | مطلوب | `string` |
| `turn` | مطلوب | `number` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:hook/result[0].data`](#persistence-type-eventhookresult0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"hook/result"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `decision` | مطلوب | `string` |
| `durationMs` | مطلوب | `number` |
| `exitCode` | اختياري | `number` |
| `handlerId` | مطلوب | `string` |
| `point` | مطلوب | `string` |
| `stderrSummary` | اختياري | `string` |
| `turn` | مطلوب | `number` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`packages/llm/llm-retry/src/types.ts#LlmRetryEventData`](#persistence-type-packagesllmllm-retrysrctypestsllmretryeventdata) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"llm/retry"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`packages/llm/llm-retry/src/types.ts#LlmRetryStartedEventData`](#persistence-type-packagesllmllm-retrysrctypestsllmretrystartedeventdata) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"llm/retry-started"` |
...
مصدر:`packages/llm/llm-retry/src/types.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `delayMs` | مطلوب | `number` |
| `failure` | مطلوب | [`packages/llm/llm/src/types.ts#LlmFailure`](#persistence-type-packagesllmllmsrctypestsllmfailure) |
| `maxRetries` | مطلوب | `number` |
| `mode` | مطلوب | `"normal"` |
| `policyKey` | مطلوب | `string` |
| `provider` | مطلوب | `string` |
| `retry` | مطلوب | `number` |
| `retryId` | مطلوب | `string` |
| `step` | مطلوب | `number` |
| `turn` | مطلوب | `number` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`packages/api/session-controller/src/types.ts#ModelSelection`](#persistence-type-packagesapisession-controllersrctypestsmodelselection) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"model/selection"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:permission/preset[0].data`](#persistence-type-eventpermissionpreset0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"permission/preset"` |
...
مصدر:`packages/interaction/permission-presets/src/index.ts`
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:plan/mode[0].data`](#persistence-type-eventplanmode0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"plan/mode"` |
...
مصدر:`packages/plan/plan-mode/src/index.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `active` | مطلوب | `boolean` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`packages/core/session/src/types.ts#RequestContext`](#persistence-type-packagescoresessionsrctypestsrequestcontext) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"request/context"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:request/header[0].data`](#persistence-type-eventrequestheader0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"request/header"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `header` | مطلوب | [`packages/core/session/src/types.ts#EpochHeader`](#persistence-type-packagescoresessionsrctypestsepochheader) |
| `reason` | مطلوب | [`packages/core/session/src/types.ts#RequestHeaderReason`](#persistence-type-packagescoresessionsrctypestsrequestheaderreason) |
| `startsSeries` | اختياري | `true` |
...
[`packages/llm/llm/src/types.ts#ToolSchema`](#persistence-type-packagesllmllmsrctypeststoolschema) عدد مجموعة.
...
| خاصية | وجود | نوع |
|---|---|---|
| [`string`] | بحث جذب توقيع | `unknown`(لا نفاذ واضح) |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:sandbox/mode[0].data`](#persistence-type-eventsandboxmode0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"sandbox/mode"` |
...
مصدر:`packages/sandbox/sandbox-policy/src/session-mode.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `mode` | مطلوب | [`packages/sandbox/sandbox/src/index.ts#SandboxMode`](#persistence-type-packagessandboxsandboxsrcindextssandboxmode) |
| `source` | اختياري | `"delegation"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`packages/schedule/schedule/src/types.ts#ScheduleChange`](#persistence-type-packagesscheduleschedulesrctypestsschedulechange) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"schedule/change"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:session-log-deepseek/delivery-accepted[0].data`](#persistence-type-eventsession-log-deepseekdelivery-accepted0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"session-log-deepseek/delivery-accepted"` |
...
مصدر:`packages/session/session-log-deepseek/src/types.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `sessionFormatVersion` | اختياري | `number` |
| `sessionId` | مطلوب | `string` |
| `throughSeq` | مطلوب | `number` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:session/end-seed[0].data`](#persistence-type-eventsessionend-seed0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"session/end-seed"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`packages/session/session-title/src/types.ts#SessionTitleEventData`](#persistence-type-packagessessionsession-titlesrctypestssessiontitleeventdata) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"session/title"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`packages/session/session-title-llm/src/index.ts#SessionTitleLlmRequestEventData`](#persistence-type-packagessessionsession-title-llmsrcindextssessiontitlellmrequesteventdata) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"session/title-llm-request"` |
...
[`packages/llm/llm/src/message.ts#Message`](#persistence-type-packagesllmllmsrcmessagetsmessage) عدد مجموعة.
...
مصدر:`packages/session/session-title/src/types.ts`
...
|---|---|---|
| `kind` | مطلوب | `"provider"` |
| `model` | اختياري | [`packages/session/session-title/src/types.ts#SessionTitleModelProvenance`](#persistence-type-packagessessionsession-titlesrctypestssessiontitlemodelprovenance) |
| `provider` | مطلوب | `string` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:step/end[0].data`](#persistence-type-eventstepend0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"step/end"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`packages/subagent/subagent/src/descriptor.ts#SubagentDescriptorData`](#persistence-type-packagessubagentsubagentsrcdescriptortssubagentdescriptordata) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"subagent/descriptor"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:subagent/model-selection-policy[0].data`](#persistence-type-eventsubagentmodel-selection-policy0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"subagent/model-selection-policy"` |
...
مصدر:`packages/subagent/tool-subagent/src/model-selection-state.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `allowedModels` | مطلوب | [`event:subagent/model-selection-policy[0].data.allowedModels`](#persistence-type-eventsubagentmodel-selection-policy0dataallowedmodels) |
...
[`packages/session/session-title/src/types.ts#SessionTitleModelProvenance`](#persistence-type-packagessessionsession-titlesrctypestssessiontitlemodelprovenance) عدد مجموعة.
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:team/member[0].data`](#persistence-type-eventteammember0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"team/member"` |
...
مصدر:`packages/experimental/agent-team/src/types.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `member` | مطلوب | [`packages/experimental/agent-team/src/types.ts#TeamMemberSnapshot`](#persistence-type-packagesexperimentalagent-teamsrctypeststeammembersnapshot) |
| `teamId` | مطلوب | `string` |
| `version` | مطلوب | `2` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:team/message/delivered[0].data`](#persistence-type-eventteammessagedelivered0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"team/message/delivered"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `messageId` | مطلوب | `string` |
| `targetId` | مطلوب | `string` |
| `teamId` | مطلوب | `string` |
| `version` | مطلوب | `2` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:team/message/queued[0].data`](#persistence-type-eventteammessagequeued0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"team/message/queued"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `message` | مطلوب | [`packages/experimental/agent-team/src/types.ts#TeamMessageSnapshot`](#persistence-type-packagesexperimentalagent-teamsrctypeststeammessagesnapshot) |
| `teamId` | مطلوب | `string` |
| `version` | مطلوب | `2` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:team/task[0].data`](#persistence-type-eventteamtask0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"team/task"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `task` | مطلوب | [`packages/experimental/agent-team/src/types.ts#TeamTaskSnapshot`](#persistence-type-packagesexperimentalagent-teamsrctypeststeamtasksnapshot) |
| `teamId` | مطلوب | `string` |
| `version` | مطلوب | `2` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:todo/write[0].data`](#persistence-type-eventtodowrite0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"todo/write"` |
...
مصدر:`packages/todo/tool-todo/src/types.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `todos` | مطلوب | [`event:todo/write[0].data.todos`](#persistence-type-eventtodowrite0datatodos) |
...
[`packages/todo/tool-todo/src/types.ts#TodoItem`](#persistence-type-packagestodotool-todosrctypeststodoitem) عدد مجموعة.
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`packages/workflow/tool-workflow/src/types.ts#ToolWorkflowAgentEndData`](#persistence-type-packagesworkflowtool-workflowsrctypeststoolworkflowagentenddata) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"tool-workflow/agent-end"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`packages/workflow/tool-workflow/src/types.ts#ToolWorkflowAgentStartData`](#persistence-type-packagesworkflowtool-workflowsrctypeststoolworkflowagentstartdata) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"tool-workflow/agent-start"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`packages/workflow/tool-workflow/src/types.ts#ToolWorkflowRunEndData`](#persistence-type-packagesworkflowtool-workflowsrctypeststoolworkflowrunenddata) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"tool-workflow/run-end"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`packages/workflow/tool-workflow/src/types.ts#ToolWorkflowRunStartData`](#persistence-type-packagesworkflowtool-workflowsrctypeststoolworkflowrunstartdata) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"tool-workflow/run-start"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:tool/call[0].data`](#persistence-type-eventtoolcall0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"tool/call"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`packages/core/tools/src/types.ts#PtcDispatchEventData`](#persistence-type-packagescoretoolssrctypestsptcdispatcheventdata) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"tool/code-dispatch"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`packages/core/tools/src/types.ts#PtcDispatchStartEventData`](#persistence-type-packagescoretoolssrctypestsptcdispatchstarteventdata) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"tool/code-dispatch-start"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:tool/result.data`](#persistence-type-eventtoolresultdata) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `sourceEventSeqs` | اختياري | [`SessionEventEnvelope[48].sourceEventSeqs`](#persistence-type-sessioneventenvelope48sourceeventseqs) |
| `surfaceOp` | اختياري | [`SessionEventEnvelope[48].surfaceOp`](#persistence-type-sessioneventenvelope48surfaceop) |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"tool/result"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `error` | اختياري | [`event:tool/result.data.error`](#persistence-type-eventtoolresultdataerror) |
| `message` | مطلوب | [`packages/llm/llm/src/message.ts#ToolResultMessage`](#persistence-type-packagesllmllmsrcmessagetstoolresultmessage) |
| `meta` | اختياري | [`event:tool/result.data.meta`](#persistence-type-eventtoolresultdatameta) |
| `step` | مطلوب | `number` |
| `turn` | مطلوب | `number` |
...
| موضع | وجود | نوع |
|---|---|---|
| 0 | مطلوب | [`packages/llm/llm/src/types.ts#ToolResultBlock`](#persistence-type-packagesllmllmsrctypeststoolresultblock) |
...
مصدر:`packages/util/values/src/index.ts`
...
[`event:tool/result.data.meta`](#persistence-type-eventtoolresultdatameta) عدد مجموعة.
...
| خاصية | وجود | نوع |
|---|---|---|
| [`string`] | بحث جذب توقيع | [`event:tool/result.data.meta`](#persistence-type-eventtoolresultdatameta) |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:turn/end[0].data`](#persistence-type-eventturnend0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"turn/end"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `reason` | مطلوب | [`packages/core/session/src/types.ts#TurnEndReason`](#persistence-type-packagescoresessionsrctypeststurnendreason) |
| `turn` | مطلوب | `number` |
...
|---|---|---|
| `kind` | مطلوب | `"aborted"` |
| `reason` | مطلوب | [`packages/core/session/src/types.ts#TurnEndCancelCause`](#persistence-type-packagescoresessionsrctypeststurnendcancelcause) |
...
| خاصية | وجود | نوع |
|---|---|---|
| `error` | مطلوب | [`packages/llm/llm/src/types.ts#LlmFailure`](#persistence-type-packagesllmllmsrctypestsllmfailure) |
| `kind` | مطلوب | `"error"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`event:turn/start[0].data`](#persistence-type-eventturnstart0data) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"turn/start"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`packages/llm/llm/src/message.ts#UserMessage`](#persistence-type-packagesllmllmsrcmessagetsusermessage) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `sourceEventSeqs` | اختياري | [`SessionEventEnvelope[48].sourceEventSeqs`](#persistence-type-sessioneventenvelope48sourceeventseqs) |
| `surfaceOp` | اختياري | [`SessionEventEnvelope[48].surfaceOp`](#persistence-type-sessioneventenvelope48surfaceop) |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"user/message"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `data` | مطلوب | [`packages/web/web-search-deepseek/src/provider.ts#DeepSeekSearchLlmRequest`](#persistence-type-packageswebweb-search-deepseeksrcprovidertsdeepseeksearchllmrequest) |
| `ignorable` | اختياري | `true` |
| `seq` | مطلوب | `number` |
| `time` | مطلوب | `number` |
| `type` | مطلوب | `"web/deepseek-search-llm-request"` |
...
مصدر:`packages/web/web-search-deepseek/src/provider.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `max_tokens` | مطلوب | `number` |
| `messages` | مطلوب | [`event:web/deepseek-search-llm-request[0].data.body.messages`](#persistence-type-eventwebdeepseek-search-llm-request0databodymessages) |
| `model` | مطلوب | `string` |
| `tools` | مطلوب | [`event:web/deepseek-search-llm-request[0].data.body.tools`](#persistence-type-eventwebdeepseek-search-llm-request0databodytools) |
...
| موضع | وجود | نوع |
|---|---|---|
| 0 | مطلوب | [`event:web/deepseek-search-llm-request[0].data.body.messages[0]`](#persistence-type-eventwebdeepseek-search-llm-request0databodymessages0) |
...
| خاصية | وجود | نوع |
|---|---|---|
| `content` | مطلوب | [`event:web/deepseek-search-llm-request[0].data.body.messages[0].content`](#persistence-type-eventwebdeepseek-search-llm-request0databodymessages0content) |
| `role` | مطلوب | `"user"` |
...
| موضع | وجود | نوع |
|---|---|---|
| 0 | مطلوب | [`event:web/deepseek-search-llm-request[0].data.body.messages[0].content[0]`](#persistence-type-eventwebdeepseek-search-llm-request0databodymessages0content0) |
...
مصدر:`packages/llm/llm/src/types.ts` · `packages/web/web-search-deepseek/src/provider.ts`
...
| موضع | وجود | نوع |
|---|---|---|
| 0 | مطلوب | [`event:web/deepseek-search-llm-request[0].data.body.tools[0]`](#persistence-type-eventwebdeepseek-search-llm-request0databodytools0) |
...
| خاصية | وجود | نوع |
|---|---|---|
| `max_uses` | مطلوب | `number` |
| `name` | مطلوب | `"web_search"` |
| `type` | مطلوب | `"web_search_20250305"` |
...
| `model` | مطلوب | `string` |
| `provider` | مطلوب | `string` |
| `reasoningEffort` | اختياري | `string` |
...
| `bytes` | مطلوب | `number` |
| `height` | مطلوب | `number` |
| `mediaType` | مطلوب | [`packages/attachment/attachment/src/types.ts#ImageMediaType`](#persistence-type-packagesattachmentattachmentsrctypestsimagemediatype) |
| `name` | اختياري | `string` |
| `originalDimensions` | اختياري | [`event:agent/inbox/spliced[0].data.inserted[0].content[0][2].attachment.originalDimensions`](#persistence-type-eventagentinboxspliced0datainserted0content02attachmentoriginaldimensions) |
| `width` | مطلوب | `number` |
...
مصدر:`packages/context/agent-instructions/src/render.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `action` | مطلوب | [`event:agent/inbox/spliced[0].data.inserted[0].source[17].changes[0].action`](#persistence-type-eventagentinboxspliced0datainserted0source17changes0action) |
| `digest` | اختياري | `string` |
| `path` | مطلوب | `string` |
| `scope` | مطلوب | `string` |
...
مصدر:`packages/context/agent-instructions/src/state.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `baseline` | اختياري | `true` |
| `baselineIdentity` | اختياري | `string` |
| `changes` | مطلوب | [`event:agent/inbox/spliced[0].data.inserted[0].source[17].changes`](#persistence-type-eventagentinboxspliced0datainserted0source17changes) |
| `form` | مطلوب | `"instructions"` |
| `kind` | مطلوب | `"agent-instructions"` |
...
|---|---|---|
| `form` | مطلوب | `"recall"` |
| `kind` | مطلوب | `"session-reference"` |
| `references` | مطلوب | [`event:agent/inbox/spliced[0].data.inserted[0].source[15].references`](#persistence-type-eventagentinboxspliced0datainserted0source15references) |
| `version` | مطلوب | `1` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `adapterDefaults` | اختياري | [`packages/llm/llm/src/call-config.ts#LlmCallConfigAdapterDefaults`](#persistence-type-packagesllmllmsrccall-configtsllmcallconfigadapterdefaults) |
| `config` | مطلوب | [`packages/llm/llm/src/call-config.ts#LlmCallConfig`](#persistence-type-packagesllmllmsrccall-configtsllmcallconfig) |
| `system` | اختياري | `string` |
| `tools` | اختياري | [`event:request/header[0].data.header.tools`](#persistence-type-eventrequestheader0dataheadertools) |
...
| خاصية | وجود | نوع |
|---|---|---|
| `agentPreset` | اختياري | `string` |
| `createdAt` | مطلوب | `number` |
| `cwd` | اختياري | `string` |
| `delegationDepth` | اختياري | `number` |
| `id` | مطلوب | `string` |
| `isSeeded` | مطلوب | `boolean` |
| `origin` | اختياري | `"subagent"` |
| `parentSession` | اختياري | `string` |
| `version` | مطلوب | `2` |
...
مصدر:`packages/core/tools/src/index.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `allow` | اختياري | [`event:assistant/attempt[0].data.stream[0][0].texts`](#persistence-type-eventassistantattempt0datastream00texts) |
| `deny` | اختياري | [`event:assistant/attempt[0].data.stream[0][0].texts`](#persistence-type-eventassistantattempt0datastream00texts) |
...
مصدر:`packages/core/tools/src/types.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `arguments` | مطلوب | `unknown`(لا نفاذ واضح) |
| `content` | مطلوب | [`event:agent/inbox/spliced[0].data.inserted[0].content`](#persistence-type-eventagentinboxspliced0datainserted0content) |
| `isError` | مطلوب | `boolean` |
| `name` | مطلوب | `string` |
| `parentCallId` | مطلوب | `string` |
| `rootCallId` | مطلوب | `string` |
| `subCallId` | مطلوب | `string` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `context` | مطلوب | [`event:team/member[0].data.member.context`](#persistence-type-eventteammember0datamembercontext) |
| `description` | مطلوب | `string` |
| `error` | اختياري | `string` |
| `id` | مطلوب | `string` |
| `name` | مطلوب | `string` |
| `phase` | مطلوب | [`packages/experimental/agent-team/src/types.ts#TeamMemberPhase`](#persistence-type-packagesexperimentalagent-teamsrctypeststeammemberphase) |
| `provider` | مطلوب | `string` |
...
| `content` | مطلوب | [`event:agent/inbox/spliced[0].data.inserted[0].content`](#persistence-type-eventagentinboxspliced0datainserted0content) |
| `id` | مطلوب | `string` |
| `senderId` | مطلوب | `string` |
| `senderName` | مطلوب | `string` |
| `targetId` | مطلوب | `string` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `blockedBy` | مطلوب | [`event:assistant/attempt[0].data.stream[0][0].texts`](#persistence-type-eventassistantattempt0datastream00texts) |
| `description` | مطلوب | `string` |
| `id` | مطلوب | `string` |
| `ownerId` | اختياري | `string` |
| `revision` | مطلوب | `number` |
| `status` | مطلوب | [`packages/experimental/agent-team/src/types.ts#TeamTaskStatus`](#persistence-type-packagesexperimentalagent-teamsrctypeststeamtaskstatus) |
| `subject` | مطلوب | `string` |
| `writeScopes` | مطلوب | [`event:assistant/attempt[0].data.stream[0][0].texts`](#persistence-type-eventassistantattempt0datastream00texts) |
...
مصدر:`packages/feedback/message-feedback/src/types.ts`
...
| `messageId` | مطلوب | `string` |
| `note` | اختياري | `string` |
| `rating` | مطلوب | [`packages/feedback/message-feedback/src/types.ts#MessageFeedbackRating`](#persistence-type-packagesfeedbackmessage-feedbacksrctypestsmessagefeedbackrating) |
| `updatedAt` | مطلوب | `number` |
| `version` | مطلوب | `string` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `item` | مطلوب | [`packages/feedback/message-feedback/src/types.ts#MessageFeedbackItem`](#persistence-type-packagesfeedbackmessage-feedbacksrctypestsmessagefeedbackitem) |
| `sessionId` | مطلوب | `string` |
...
مصدر:`packages/goal/goal/src/domain.ts`
...
| خاصية | وجود | نوع |
|---|---|---|
| `cleared` | مطلوب | [`packages/goal/goal/src/types.ts#GoalRef`](#persistence-type-packagesgoalgoalsrctypestsgoalref) |
| `clearedAt` | مطلوب | `number` |
| `kind` | مطلوب | `"goal/change"` |
| `operation` | مطلوب | `"clear"` |
| `version` | مطلوب | `1` |
...
|---|---|---|
| `createdAt` | مطلوب | `number` |
| `goal` | مطلوب | [`packages/goal/goal/src/types.ts#GoalSnapshot`](#persistence-type-packagesgoalgoalsrctypestsgoalsnapshot) |
| `kind` | مطلوب | `"goal/change"` |
| `operation` | مطلوب | [`event:goal/change[0].data[0].operation`](#persistence-type-eventgoalchange0data0operation) |
| `roundsStarted` | مطلوب | `number` |
| `updatedAt` | مطلوب | `number` |
| `version` | مطلوب | `1` |
...
مصدر:`packages/goal/goal/src/types.ts`
...
| خاصية | وجود | نوع |
|---|---|---|
| `blockedReason` | اختياري | [`packages/goal/goal/src/types.ts#GoalBlockReason`](#persistence-type-packagesgoalgoalsrctypestsgoalblockreason) |
| `id` | مطلوب | `string` |
| `maxGoalRounds` | مطلوب | `number` |
| `objective` | مطلوب | `string` |
| `phase` | مطلوب | [`packages/goal/goal/src/types.ts#GoalPhase`](#persistence-type-packagesgoalgoalsrctypestsgoalphase) |
| `revision` | مطلوب | `number` |
...
مصدر:`packages/llm/llm/src/call-config.ts`
...
| `provider` | مطلوب | `string` |
| `reasoningEffort` | اختياري | `string` |
| `stop` | اختياري | [`event:assistant/attempt[0].data.stream[0][0].texts`](#persistence-type-eventassistantattempt0datastream00texts) |
| `temperature` | اختياري | `number` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `maxTokens` | اختياري | `true` |
| `reasoningEffort` | اختياري | `true` |
...
مصدر:`packages/llm/llm/src/message.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `content` | مطلوب | [`event:agent/inbox/spliced[0].data.inserted[0].content`](#persistence-type-eventagentinboxspliced0datainserted0content) |
| `id` | مطلوب | `string` |
| `role` | مطلوب | `"assistant"` |
| `source` | مطلوب | [`packages/llm/llm/src/message.ts#ModelMessageSource`](#persistence-type-packagesllmllmsrcmessagetsmodelmessagesource) |
...
| `content` | مطلوب | [`event:agent/inbox/spliced[0].data.inserted[0].content`](#persistence-type-eventagentinboxspliced0datainserted0content) |
| `id` | مطلوب | `string` |
| `role` | مطلوب | [`event:session/title-llm-request[0].data.messages[0].role`](#persistence-type-eventsessiontitle-llm-request0datamessages0role) |
| `source` | مطلوب | [`packages/llm/llm/src/message.ts#MessageSource`](#persistence-type-packagesllmllmsrcmessagetsmessagesource) |
...
| `model` | مطلوب | `string` |
| `provider` | مطلوب | `string` |
| `replayState` | اختياري | `unknown`(لا نفاذ واضح) |
...
| خاصية | وجود | نوع |
|---|---|---|
| `content` | مطلوب | [`event:tool/result.data.message.content`](#persistence-type-eventtoolresultdatamessagecontent) |
| `id` | مطلوب | `string` |
| `role` | مطلوب | `"user"` |
| `source` | مطلوب | [`packages/llm/llm/src/message.ts#ToolMessageSource`](#persistence-type-packagesllmllmsrcmessagetstoolmessagesource) |
...
| خاصية | وجود | نوع |
|---|---|---|
| `attachment` | مطلوب | [`packages/attachment/attachment/src/types.ts#FileAttachmentRef`](#persistence-type-packagesattachmentattachmentsrctypestsfileattachmentref) |
| `type` | مطلوب | `"file"` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `attachment` | مطلوب | [`packages/attachment/attachment/src/types.ts#ImageAttachmentRef`](#persistence-type-packagesattachmentattachmentsrctypestsimageattachmentref) |
| `type` | مطلوب | `"image"` |
...
| `code` | مطلوب | `string` |
| `message` | مطلوب | `string` |
| `providerRetryAfterMs` | اختياري | `number` |
| `requestId` | اختياري | `string` |
| `status` | اختياري | `number` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `blocks` | اختياري | [`event:assistant/attempt[0].data.stream[0][3].chunk[6].replayState.blocks`](#persistence-type-eventassistantattempt0datastream03chunk6replaystateblocks) |
| `response` | مطلوب | `unknown`(لا نفاذ واضح) |
...
| خاصية | وجود | نوع |
|---|---|---|
| `cacheReadTokens` | اختياري | `number` |
| `cacheWriteTokens` | اختياري | `number` |
| `inputTokens` | مطلوب | `number` |
| `outputTokens` | مطلوب | `number` |
| `reasoningTokens` | اختياري | `number` |
| `totalTokens` | اختياري | `number` |
...
|---|---|---|
| `content` | مطلوب | [`event:agent/inbox/spliced[0].data.inserted[0].content`](#persistence-type-eventagentinboxspliced0datainserted0content) |
| `isError` | اختياري | `boolean` |
| `toolCallId` | مطلوب | `string` |
| `type` | مطلوب | `"tool-result"` |
...
| `description` | مطلوب | `string` |
| `name` | مطلوب | `string` |
| `parameters` | مطلوب | [`event:request/header[0].data.header.tools[0].parameters`](#persistence-type-eventrequestheader0dataheadertools0parameters) |
...
مصدر:`packages/sandbox/sandbox/src/index.ts`
...
مصدر:`packages/schedule/schedule/src/types.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `afterSeconds` | مطلوب | `number` |
| `id` | مطلوب | `string` |
| `kind` | مطلوب | `"after"` |
| `prompt` | مطلوب | `string` |
| `scheduledAt` | مطلوب | `string` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `acceptedAt` | مطلوب | `string` |
| `id` | مطلوب | `string` |
| `operation` | مطلوب | `"dispatch"` |
| `version` | مطلوب | `1` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `operation` | مطلوب | `"create"` |
| `schedule` | مطلوب | [`packages/schedule/schedule/src/types.ts#ScheduleRecord`](#persistence-type-packagesscheduleschedulesrctypestsschedulerecord) |
| `version` | مطلوب | `1` |
...
مصدر:`packages/session/session-persistence-jsonl/src/format.ts`
...
| `createdAt` | مطلوب | `number` |
| `cwd` | اختياري | `string` |
| `delegationDepth` | مطلوب | `number` |
| `id` | مطلوب | `string` |
| `isSeeded` | مطلوب | `boolean` |
| `origin` | اختياري | `"subagent"` |
| `parentSession` | اختياري | `string` |
| `type` | مطلوب | `"session"` |
| `version` | مطلوب | `number` |
...
مصدر:`packages/session/session-title-llm/src/index.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `maxTokens` | مطلوب | `number` |
| `messageSeqs` | مطلوب | [`SessionEventEnvelope[48].sourceEventSeqs`](#persistence-type-sessioneventenvelope48sourceeventseqs) |
| `messages` | مطلوب | [`event:session/title-llm-request[0].data.messages`](#persistence-type-eventsessiontitle-llm-request0datamessages) |
| `route` | مطلوب | [`packages/session/session-title/src/types.ts#SessionTitleModelProvenance`](#persistence-type-packagessessionsession-titlesrctypestssessiontitlemodelprovenance) |
| `system` | مطلوب | `string` |
| `titleProvider` | مطلوب | `string` |
...
|---|---|---|
| `messageSeqs` | مطلوب | [`SessionEventEnvelope[48].sourceEventSeqs`](#persistence-type-sessioneventenvelope48sourceeventseqs) |
| `source` | مطلوب | [`packages/session/session-title/src/types.ts#SessionTitleSource`](#persistence-type-packagessessionsession-titlesrctypestssessiontitlesource) |
| `title` | مطلوب | `string` |
...
مصدر:`packages/session/session-title/src/types.ts` · `packages/subagent/tool-subagent/src/model-selection.ts`
...
مصدر:`packages/skill/skill/src/index.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `form` | مطلوب | `"instructions"` |
| `kind` | مطلوب | `"skill-invocation"` |
| `name` | مطلوب | `string` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `entries` | مطلوب | [`event:agent/inbox/spliced[0].data.inserted[0].source[16].entries`](#persistence-type-eventagentinboxspliced0datainserted0source16entries) |
| `form` | مطلوب | `"catalog"` |
| `kind` | مطلوب | `"skill-catalog"` |
| `update` | اختياري | `true` |
...
مصدر:`packages/subagent/subagent/src/continuation-messages.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `form` | مطلوب | `"relay"` |
| `kind` | مطلوب | `"agent-message"` |
| `senderSessionId` | مطلوب | `string` |
...
مصدر:`packages/subagent/subagent/src/descriptor.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `agentModel` | اختياري | `string` |
| `agentProvider` | اختياري | `string` |
| `agentReasoningEffort` | اختياري | `string` |
| `label` | مطلوب | `string` |
| `mode` | مطلوب | `"continuable"` |
| `persona` | اختياري | `string` |
| `provider` | مطلوب | `string` |
| `toolFilter` | اختياري | [`packages/core/tools/src/index.ts#ToolRestriction`](#persistence-type-packagescoretoolssrcindextstoolrestriction) |
| `version` | مطلوب | `number` |
...
| `status` | مطلوب | [`event:todo/write[0].data.todos[0].status`](#persistence-type-eventtodowrite0datatodos0status) |
...
| خاصية | وجود | نوع |
|---|---|---|
| `apiVersion` | مطلوب | `string` |
| `body` | مطلوب | [`event:web/deepseek-search-llm-request[0].data.body`](#persistence-type-eventwebdeepseek-search-llm-request0databody) |
| `endpoint` | مطلوب | `string` |
...
مصدر:`packages/workflow/tool-workflow/src/types.ts`

| خاصية | وجود | نوع |
|---|---|---|
| `outcome` | مطلوب | [`packages/workflow/workflow/src/types.ts#WorkflowAgentOutcome`](#persistence-type-packagesworkflowworkflowsrctypestsworkflowagentoutcome) |
| `runId` | مطلوب | `string` |
| `seq` | مطلوب | `number` |
...
| خاصية | وجود | نوع |
|---|---|---|
| `childId` | مطلوب | `string` |
| `label` | مطلوب | `string` |
| `phase` | اختياري | `string` |
| `runId` | مطلوب | `string` |
| `seq` | مطلوب | `number` |
...
|---|---|---|
| `runId` | مطلوب | `string` |
| `stopReason` | مطلوب | [`packages/workflow/workflow/src/types.ts#WorkflowStopReason`](#persistence-type-packagesworkflowworkflowsrctypestsworkflowstopreason) |
...
مصدر:`packages/workflow/workflow/src/types.ts`
...
مصدر:`packages/api/session-controller/src/types.ts` · `packages/attachment/attachment/src/brand.ts` · `packages/compaction/compaction/src/brand.ts` · `packages/core/session/src/types.ts` · `packages/experimental/agent-team/src/types.ts` · `packages/feedback/message-feedback/src/types.ts` · `packages/goal/goal/src/types.ts` · `packages/interaction/commands/src/brand.ts` · `packages/interaction/user-approval/src/types.ts` · `packages/llm/llm-retry/src/brand.ts` · `packages/llm/llm/src/brand.ts` · `packages/schedule/schedule/src/types.ts` · `packages/session/session-title/src/index.ts` · `packages/session/session-title/src/types.ts` · `packages/webhook/webhook/src/brand.ts` · `packages/workflow/workflow/src/types.ts`
...
`unknown`: هذا إعلان لم كشف تخزين قيمة داخلي حقل.
...
[فقط استخدام شفرة المصدر رفع أخذ جهاز](../../../scripts/persistence-schema.ts) بـ TypeScript 6.0.3 تشغيل. إعادة بناء وقت حذف زائد بقية `object` تسليم تقاطع نوع عضو، و إسقاط مواصفة تحويل بعد غير ممكن بلوغ نوع تعريف؛ إبقاء قد لديه `any` / `unknown`، لا بـ لا نفاذ واضح نوع بديل لم تحليل استيراد.V0 و V2 أصل نوع ملخص و من مسبق إصدار عودة ملف إعادة بناء مقابل كامل حالة متسق.

تاريخ surface حدث إعلان سماح اختياري `surfaceOp`. هذا إبقاء نوع إعلان دليل، و لا سماح حذف codec أو ترحيل تحقق جهاز اشتراط حقل. لا نفاذ واضح قيمة، خارجي إضافة حدث، تحزيم body تحرير رمز و وقت التشغيل علاقة متساو لا في نوع دليل تغطية نطاق داخل.

```sh
pnpm run verify-persistence-formats
```

هذا فحص فقط استخدام هذا عمل شجرة تحقق مباشر إلى حالي كتابة جهاز كل صيغة،schema كامل صفة، مزدوج لغة إعلان و توليد جدول إطار. هو لن إعادة تشغيل تاريخ تطبيق، أيضا لا تأكيد قيام ترحيل أمان صفة. صيانة قاعدة مشاركة رؤية[صيغة بحث جذب](README.zh.md) ، حالي إعلان مشاركة رؤية[حالي دليل](../../persistence-catalog.zh.md).
