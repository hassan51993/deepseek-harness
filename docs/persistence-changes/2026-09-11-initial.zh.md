...
# ابتدائي حفظ دائم نوع أساس خط

[English](2026-09-11-initial.md) | العربية

## عام وصف

هذا أساس خط سجل حالي شفرة المصدر إعلان كامل حفظ دائم نوع دليل، لـ لاحق نوع تحويل توفير حفظ في هذا شفرة المصدر شجرة داخل بدء نقطة.
...
هذا سجل ابتدائي تحويل نوع تاريخ، لا إعلان أكثر مبكر إصدار نوع علاقة، أيضا لا تغيير Session writer إصدار أو وقت التشغيل سلوك. كل أصل نوع من فارغ قبل دفع بدء، و إبقاء كامل مواصفة تحويل schema؛ لاحق سجل مرجع هذا سجل after schema.

<a id="verification"></a>
## تحقق

قد تشغيل `pnpm exec tsx scripts/gen-persistence-catalog.ts` توليد حالي دليل، و تشغيل `pnpm run persistence-changes --baseline 2026-09-11-initial` إنشاء هذا أساس خط و ذلك schema لقطة.
