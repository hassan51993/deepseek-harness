...
مجموعة يمكن تضمين طقم واحد نسخة Cordis بند إعداد فرعي قائمة، و سوف ذلك بصفة واحد وحدة تحميل و إزالة؛`isolate` فإن لـ واحد مجموعة توفير بعض بند خدمة اسم مستقل نسخة، لذلك اثنان عدد مجموعة يمكن كل منها يرى إعداد مختلف `shell` مزود، متبادل لا أثر.[Cordis دخول باب](../cordis-primer.ar.md) و[خدمة عزل عرض مثال](../user/develop/framework/service.ar.md#service-isolation) وسيط تعريف تفصيل دقيق محتوى.

## حار وحدة استبدال

إزالة سوف تحرير effect([رقم 2 فصل](02-lifecycle-and-effects.ar.md)) ، تحميل فإن التزام دوران اعتماد علاقة ([رقم 3 فصل](03-services.ar.md)) ، لذلك HMR يمكن أولا إزالة، مجددا تحميل، بـ استبدال جارٍ تشغيل إضافة.`@deepseek-ai/dsh-hmr` إضافة سوف مراقبة نظر ملف، و في حفظ وقت تنفيذ هذا واحد مرور مسار.
...
قائمة في زيادة اثنان عدد مساعد مساعدة إضافة:HMR عبر Cordis logger خدمة سجل سجل، لذلك لا يوجد تحكم منصة تصدير جهاز وقت نظر لا إلى ذلك رسالة؛ هو أيضا سوف `inject` `timer` خدمة قدوم تنفيذ ذهاب اهتزاز، إذا لا يوجد `@deepseek-ai/cordis-plugin-timer`، هو حينئذ سوف دائم بعيد توقف في PENDING، بينما كما لا إرسال خروج أي تلميح. تحت واحد عقدة حينئذ نقاش نقاش هذا نوع ساكن صامت حالة.
...
أنت يمكن مباشر عرض هذه حالة. كل سياق كل قدرة قطعة رفع إضافة سجل التسجيل؛ إنشاء `diagnose.ts`:
...
تحت واحد فصل:[دخول harness](07-into-the-harness.ar.md): يأخذ نفسه نمط لأجل حقيقي harness خدمة.
