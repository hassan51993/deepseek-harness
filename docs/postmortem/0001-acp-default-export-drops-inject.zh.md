...
`packages/acp/acp/src/index.ts` هو واحد*نطاق الأسماء إضافة*: هو سوف `name`،`inject`،`Config` و `apply` بصفة مستقل تسمية توجيه خروج، مستودع في أخرى كل إضافة (`invariants`،`llm-deepseek`،`tool-bash`،`tui` انتظار) أيضا هو مثل هذا. لكن هو*أيضا*كثير واحد سطر أخرى إضافة كل لا يوجد شفرة:
...
وجود افتراضي توجيه خروج وقت،`exports.default ?? exports` تحليل لـ**عار `apply` دالة**. عار دالة لا يوجد `inject`، لا يوجد `name`، لا يوجد `Config` خاصية——هذه بصفة*نفس درجة*تسمية توجيه خروج وجود في وحدة نطاق الأسماء فوق، بينما unwrap إلى `.default` يأخذ كامل نطاق الأسماء إسقاط.Loader مع بعد أساس في فارغ `inject` بناء إضافة fiber.
...
- **[docs/testing.md](../testing.zh.md) قاعدة**:«اختبار حقيقي مدخل مسار» ، سطر نسبة التغطية لا انتظار في سلوك نسبة التغطية——سوف هذا واحد تعليم تدريب تحرير تأليف لـ كل لم قدوم إضافة قاعدة.
...
- متبادل معلومة تتبع أثر نتيجة، لا يلزم حيرة معلومة إدارة نقاش. أفضل أنيق shadow حل تفسير هو حقيقي، لكن هو هو*ثاني عدد* bug؛*رقم واحد*هو واحد سطر توجيه خروج خطأ، في عدد صغير وقت نظر يشبه دمج إدارة لكن فعلي خطأ دفع إدارة بعد، واحد fiber مرة تاريخ `console.error` في بضعة قسم ساعة داخل حينئذ بحث إلى هو.
