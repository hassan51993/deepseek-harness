...
هذا تعليم مسار سوف إنشاء واحد الأكثر صغير Harness إضافة، و سوف ذلك تحميل إلى Web UI في. طلب من اكتمل[من شفرة المصدر تشغيل مسار](../../../../README.ar.md#run-from-source) مستودع فحص خروج بدء.
...
في Harness في، إضافة هو واحد تصدير `apply` دالة TypeScript وحدة. إطار هيكل في تحميل وقت استدعاء `apply`، نقل دخول واحد `ctx`(سياق كائن) ، أنت عبر `ctx` تسجيل قدرة:
...
إذا أنت لديه حاجة يد حركة تنظيف مورد (مقارنة مثل واحد شبكة اتصال) ، استخدام `ctx.effect()` إبلاغ إبلاغ إطار هيكل كيف ما تنظيف:
