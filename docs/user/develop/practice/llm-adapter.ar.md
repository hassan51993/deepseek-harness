...
`stream()` استقبال مستودع تصدير `GenerateOptions`. هو يتضمن نموذج، مهايئ يملك دفع إدارة قوي درجة ID، محادثة تاريخ، توجيه النظام، أداة schema، توليد معامل، إيقاف تسلسل و في توقف إشارة؛ كامل حقل بـ `@deepseek-ai/dsh-llm` تصدير TypeScript نوع لـ دقيق. مهايئ يجب سوف دعم حمل حقل خريطة إلى أداة جسم API؛ إذا لا يمكن دعم حمل بعض عدد حقل، ينبغي رمي خروج حمل مستقر code `LlmError`، لا نيل ساكن صامت إسقاط.
