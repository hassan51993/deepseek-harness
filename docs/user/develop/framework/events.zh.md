...
Harness Cordis حدث التزام دوران `namespace/action` تسمية، مثال مثل `agent/pre-step`،`agent/request`،`agent/request-error`،`tools/result` و `session/event`. كامل توقيع و إطلاق نمط رؤية[فرعي نظام صفحة](../../../subsystems/core.zh.md) فوق توليد `cordis-surface` منطقة كتلة.
...
- [قدرة قسم طبقة](../practice/index.zh.md) — حل قدرة واجهة في حدث
- [LLM(كبير لغة نموذج) مهايئ](../practice/llm-adapter.zh.md) — تنفيذ واحد كامل LLM خلفية
