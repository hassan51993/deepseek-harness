...
كل متن توسيع مستقل يحتفظ ذاته `version`. إصدار فقط ملائم لأجل يتضمن هذا حقل كائن؛ مختلف حقل إصدار بين لا وجود توافق أو ترتيب علاقة.JSON عضو ترتيب لا يخص بروتوكول.

[`DeepSeekLlmApiExtensionRegistry`](../packages/llm/deepseek-llm-api-extensions/README.ar.md) لـ كل قمة طبقة توسيع اسم إبقاء واحد مزود. فارغ اسم، اثنان طرف حمل فارغ أبيض اسم، تكرار تسجيل و و DeepSeek أساس أساس طلب اندفاع مفاجئ اسم كل سوف في HTTP قسم إرسال قبل فشل.
...
[`@deepseek-ai/dsh-plugin-package-inventory-deepseek`](../packages/llm/plugin-package-inventory-deepseek/README.ar.md) مساهمة كامل تخزين نشط Loader-backed إضافة حزمة بيان. هذا حقل افتراضي تفعيل.
...
|---|---|---|
| `version` | مطلوب | قدوم ذاتي `Session.header` منطق Session صيغة إصدار؛ رؤية[صيغة حالة](session-format-status.ar.md) |
| `id` | مطلوب | تأكيد قطع جلسة id |
| `createdAt` | مطلوب | غير سالب أمان كامل عدد Unix epoch مللي ثانية عدد |
