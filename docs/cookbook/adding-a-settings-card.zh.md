...
متصفح نصف جانب من[عميل وحدة نظام](../../packages/client/modules) توفير إعطاء صفحة: هو مسح قد تفعيل Loader entries في إعلان `dsh.client` حزمة، و توفير كل حزمة بناء خروج `./client` توجيه خروج. لذلك فقط يلزم `cordis.yml` تركيب هذا إضافة، هو حينئذ سوف ظهور في صفحة فوق——بلا حاجة إعادة بناء Web تطبيق.
