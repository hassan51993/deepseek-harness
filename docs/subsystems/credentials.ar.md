...
يُبقي seam الاعتمادات في [dsh-credentials](../../packages/credentials/credentials) الأسرارَ خارج الضبط: فأقسامُ الإعدادات ومداخلُ `cordis.yml` تحمل *مراجع* (أسماءَ متغيرات بيئة)، ويملك المزوّدون مثل [dsh-credentials-local](../../packages/credentials/credentials-local) القيمَ، ويحلّ المستهلكون المرجعَ مرةً واحدة لكل عملية — وتحلّه مهايئاتُ LLM مرةً لكل طلب نموذج، فيصل الاعتمادُ المُدوَّر إلى الطلب التالي مباشرةً بلا إعادة تشغيل. وقاعدةٌ واحدة تسري على كل مزوّد في هذا الـ seam: القيمةُ المخزَّنة الفارغة غائبةٌ في كل مكان.

المصدر: [`packages/credentials/credentials/src/index.ts`](../../packages/credentials/credentials/src/index.ts)

## الهوية

يسمّي المرجعُ اعتمادًا واحدًا باسم متغير بيئة على نسق POSIX. وتمنع العلامةُ المستدعين من خلط مراجع الاعتمادات بسلاسل أخرى تُمرَّر بين الحزم أو العمليات؛ ويتحقق الإنشاءُ من نحو معرّف الصدفة.
...
يعيد `resolve(ref)` القيمةَ ومعها طبقةُ المصدر التي يعرّفها المزوّد وقدّمتها، أو يعيد `undefined` ما دامت غير مضبوطة. ويعيد المستهلكون الحلَّ عند كل عملية ولا يخزّنونه عبر العمليات — وهذه القراءةُ لكل عملية هي آليةُ التحديث الساخن.
...
يجيب `describe(ref)` واجهاتِ الضبط بلا كشف أي قيمة: أيحلّ المرجعُ أم لا، ومن أي طبقة، وهل ينجح `set` الآن. ويبلّغ المزوّدُ المحلي عن مرجع تقدّمه بيئةُ العملية الحية بأنه `writable: false` — لأن الكتابةَ ستبدو ناجحةً بينما يظل الحلُّ يعيد القيمةَ الحاجبة، فيرفضها الـ seam وتستطيع الواجهةُ عرضَ المرجع للقراءة فقط من البداية.
...
ينطلق `credentials/reference-updated (ref)` بعد تغيير مُودَع في مصدر يديره مزوّد — `set` أو `unset` أو تعديل خارجي رُصد في التخزين. أما تغييراتُ بيئة العملية المحيطة فغيرُ قابلة للرصد ولا تُطلق شيئًا. ولا يحتاج المستهلكون إلى الحدث، لأنهم يعيدون الحلَّ لكل عملية؛ وإنما وُجد لواجهات الضبط التي تحدّث شارةَ «مضبوط».
