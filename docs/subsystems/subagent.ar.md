...
`subagentCatalog` projection عبر Session مراقبة و عميل لقطة كشف حسب أب جلسة حدث ترتيب ترتيب `SubagentCatalogEntry[]`. كل بند يتضمن فرعي درجة id، إنشاء وقت، نمط و اعتماد نمط تحديد وسم؛fork وراثة دليل واقع لا في منها.[subagent حزمة](../../packages/subagent/subagent/README.ar.md) تعريف دليل إنشاء و حفظ دائم دلالة.
