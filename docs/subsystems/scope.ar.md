...
`ScopeKey` هو واحد لا نفاذ واضح كائن هوية معرف. قد تسليم agent loop(ذكي جسم حلقة) استخدام نشط وثب `Agent` كائن بصفة ذاته key، لكن هذا أصل لغة من لا عرض هذا كائن.
