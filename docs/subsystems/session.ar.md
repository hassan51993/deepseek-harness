...
أساس في `type` حق صحيح يمكن تمييز تعرف ربط دمج (بينما غير مستقل `type`/`data` ربط دمج) ، لذلك `switch (event.type)` قدرة مباشر استلام ضيق `event.data`، بلا حاجة نوع تأكيد.`seq` هو سجل في مفرد ضبط تمرير زيادة موضع (`seq = log.length`) ؛`time` لـ epoch مللي ثانية.
...
`SessionEventType = keyof SessionEventMap`. من في `SessionEventMap` يمكن عبر دمج توسيع، مقابل `SessionEvent` switch لغة جملة منع توقف استخدام `assertNever`: إضافة تغيير جسم هو دمج قاعدة لم معرفة قيمة؛ معالجة معروف case بعد في `default` في وضع سطر.
...
صريح `boundary` سماح استدعاء من من مهمة معنى مستقر جولة بين موضع fork، يشمل قبل `turn/end` أو أكثر متأخر مستقل صاف سجل حدث، أي جعل مصدر جلسة لديه تحديث حدث أو جارٍ إجراء جولة.API رفض انتهاء في فتح وضع جولة داخل بادئة، بينما لا هو ساكن صامت مقتطع. أكثر واسع عام تنفيذ علاقة سليم كل صفة فحص إبقاء في قائم `dsh-invariants` إضافة و حفظ دائم إصلاح مسار في، لا في `fork()` في تكرار.`dsh-subagent-fork-in-process` إبقاء ذلك اكتمل بادئة مقتطع منطق، لأن استدعاء الأداة وقت تفويض حمل عبر معتاد في أب جولة ما زال فتح وقت بدء؛ عادي جلسة فرع ينبغي صريح إشارة تحديد طلب boundary.
...
`max-tokens` و نموذج استدعاء في نفس اسم `FinishReason` مقابل: فقط يلزم جولة داخل لديه أي خطوة بـ `max-tokens` انتهاء، كامل جولة حينئذ بـ `max-tokens` بينما لا هو `completed` انتهاء (أي جعل بعد متابعة تنفيذ، مقتطع واقع ما زال أولوية) ، يجعل مستهلك قدرة كاف منطقة قسم صحيح معتاد إيقاف و مقتطع إيقاف. إلغاء و خطأ ما زال هو مختلف نتيجة.`interrupted` هو وحيد لن من أي loop إرسال خروج سبب: هو من انهيار انهيار استعادة دمج صار (رؤية [persistence.md](persistence.ar.md)). هذا map يمكن عبر دمج توسيع.
...
حسب حق شخص نشط حركة ترتيب Session مستهلك سوف ترتيب حذف هذا حد: وصل يد Session لا حساب عمل، لذلك حسب سجل ذيل جزء ترتيب سوف يأخذ كل فتح مرور Session قمة إلى الأكثر قبل.
