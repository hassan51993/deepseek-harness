...
MCP خادم حاجة رئيسي حركة إعداد. في هدف Cordis أثر مجال في، لـ كل منصة خادم إعداد واحد `@deepseek-ai/dsh-mcp-client` بند. كل مع مرفق profile كل توفير[أداة سجل التسجيل](tools.ar.md) ، و موحد واحد تركيب مشترك مورد خدمة مرة؛ مستخدم فقط يحتاج إعداد عميل بند. استدعاء جهة لا يوجد مرئي قد إعداد خادم وقت، في native أو PTC نمط تحت كل لن نيل نيل MCP نص التوجيه نص أو أداة.

| اختيار | إعداد صيانة موضع |
|---|---|
| خادم هوية، محلي عملية أو HTTP طرف نقطة، اعتماد و عملية بيئة | [عميل إعداد](../../packages/mcp/mcp-client/README.ar.md#use-this-package) |
| أداة و مورد طلب مهلة، بدء فشل سياسة و إعادة وصل | [عميل إعداد](../../packages/mcp/mcp-client/README.ar.md#use-this-package) |
| مورد اكتشاف و قراءة | مع مرفق profile قد يتضمن [MCP مورد خدمة](../../packages/mcp/mcp-resources/README.ar.md#use-this-package) ؛ هذا خدمة لا يوجد إعداد حقل |
| خادم إشارة أمر كبير صغير حد | عميل `maxInstructionBytes`؛ تركيب توفير[توجيه النظام تركيب إعداد](system-prompt.ar.md) |
| إذن قرار و تلقي دعم حمل رسم مثل إخراج | [أداة تنفيذ](tools.ar.md) و[مرفق عنصر](attachment.ar.md) |

بروتوكول تنسيق تجارة التزام دوران SDK دعم حمل إصلاح حجز إصدار؛ منتج لا يوجد قوي صنع إشارة تحديد بروتوكول إصلاح حجز إصدار ضبط.[إعداد دليل](../config-catalog.ar.md#deepseek-aidsh-mcp-client) صف خروج عميل قبول حقل و قيمة افتراضية.
...
[أصلي Cua Driver مزود](../../packages/experimental/computer-use-cua-driver-native/README.ar.md) إعادة استخدام عميل تصدير نتيجة مهايئ، بلا حاجة فتح MCP اتصال. طاولة وجه مزود اختيار يخص[حساب حساب آلة استخدام فرعي نظام](computer-use.ar.md).
...
نتيجة مهايئ لـ برنامج تحويل استدعاء جهة إبقاء مواصفة MCP JSON، و دقيق تجهيز عادي أداة محتوى. تلقي دعم حمل رسم مثل استخدام مرفق عنصر نظام؛ لا تلقي دعم حمل غني محتوى إنتاج واضح نص تشخيص. أداة سجل التسجيل ما زال قرار سياسة فشل و نتيجة استبدال.[أداة عقد نحو](tools.ar.md) صيانة سجل و نهائي عرض قاعدة؛[عميل نتيجة مشاركة اعتبار](../../packages/mcp/mcp-client/README.ar.md#use-this-package) صيانة MCP خاص لديه إسقاط دقيق عقدة.
...
- [MCP حزمة مجموعة](../../packages/mcp/README.ar.md) — حزمة مدخل.
- [MCP مورد](../../packages/mcp/mcp-resources/README.ar.md) — مشترك أداة و مورد مزود دلالة.
...
- [رقم ثلاثة جهة تسجيل ذاكرة خادم](../user/guide/mcp-memory.ar.md) — منتج إعداد إشارة جنوب.
- [بروتوكول تنسيق تجارة قرار](../../.agents/notes/implemented/feature/2026-09-12-mcp-sdk-protocol-negotiation.ar.md) — SDK مسؤولية و توافق صفة قرار.
