# الاستعلام عن الجلسات

[English](session-query.md) | العربية

مفرداتُ استعلام فوق مجموعة الجلسات المنطقية التي يُفضَّل فيها الحي. وتملك [حزمة تعريف الخدمة](../../packages/session-query/session-query) القراءاتِ الدقيقة وأسبقيةَ المصادر وتتبعَ العلاقات والاستخراجَ الدلالي والمرشّحاتِ المستقلة عن المزوّدين، بينما تملك [مزوّدة SQLite](../../packages/session-query/session-query-sqlite) دورةَ حياة فهرس النص الكامل الملموس.

المصدر: [`packages/session-query/session-query/src/types.ts`](../../packages/session-query/session-query/src/types.ts)

## السجلات المنطقية

`SessionRecord` هو ما تعيده القائمةُ عبر المجموعة. وهو يكشف إتاحةَ المصدر مستقلةً عن الترويسة المنسوخة التي يُفضَّل فيها الحي. و`SessionEventRecord` إسقاطٌ خفيف للسجل الخام؛ ويستعمل التصنيفُ انتقالاتِ `foldSurface()` نفسَها التي يستعملها اشتقاقُ تاريخ النموذج.
...
و`SessionLogSnapshot` هو السجلُّ الخام الكامل المنفصل المتحقَّق منه بإعادة التشغيل والمستعمَل في تمهيد الاستئناف. و`SessionSurfaceSnapshot` مراقبةُ سطح واحدة من قراءة دقيقة لا اشتراكٌ محفوظ.
...
ويطبّق `SessionTitleObservation` قاعدةَ المراقبة الذرّية نفسَها على طيّ العناوين، فيستطيع مستهلكُ تخويل أن يتحقق من ترويسة المصدر التي قدّمت العنوان. وتعيد القراءاتُ المجمَّعة `SessionTitleObservationResult` واحدًا مرتَّبًا لكل معرّف مطلوب فريد: فتبقى الإخفاقاتُ التشغيلية محليةً في ذلك المعرّف، بينما يرفض الإلغاءُ العمليةَ كلَّها.
...
## المرشّحات والوثائق المستقلة عن المزوّدين

تُوصل مصفوفاتُ مرشّحات الجلسات والأحداث بـ«و»؛ وتُوصل القيمُ داخل بند القائمة الواحد بـ«أو». والمدياتُ شاملة. وبندُ `text` في الأحداث مسحٌ بتعبير نمطي حرفي بمحارف Unicode بلا حساسية لحالة الأحرف ومرنٍ تجاه الفراغات على النص الدلالي المستخرَج، مستقلًّا عن مزوّدي النص الكامل.
...
وتطبّق `ctx.sessionQuery.filterSessions(filters)` مرشّحَ `SessionResultFilter` على المجموعة المنطقية كاملةً؛ وتعيد `ctx.sessionQuery.filterEvents(sessionId, filters)` الوثائقَ المطابِقة بترتيب تسلسل تصاعدي. وتسهم الرسائلُ ونداءاتُ الأدوات ونتائجُها وقوائمُ المهام وتفاصيلُ الإخفاق والحالة بنص دلالي؛ ولا تسهم كتلُ الاستدلال ولا المطالباتُ المحجوبة ولا الأحداثُ البنيوية ولا قطعُ المجرى.

## صفحات البحث بالنص الكامل

لـseam `ctx.sessionQuery` المدمج نطاقا بحث بالنص الكامل. فـ`searchSessions()` تجمع المجموعةَ بأقوى حدث مطابِق؛ و`searchEvents()` تبحث في جلسة واحدة. وتربط الطلباتُ مؤشرًا معتمًا بالاستعلام الموحَّد ومرشّحاتِ البيانات الوصفية والحدّ. ومسحُ نص الأحداث غائبٌ عمدًا عن مرشّحات البيانات الوصفية لدى المزوّدين.
...
وخلافًا للإصابات المجمَّعة عبر الجلسات، على البحث داخل الجلسة أن يكشف أيضًا ترويسةَ الهدف المرصودة ولو لم تحتوِ الصفحةُ إصابات.
...
يحمل `SessionLineageTrace` الآباءَ المعروفين من الأقرب إلى الأبعد، وغابةً من الأحفاد المباشرين المتداخلين تكراريًّا. ويجعل مميِّزُ الاكتمال الجذرَ المعروفَ والأبَ المفقودَ متعارضين لا يجتمعان.
...
## القراءات المحدودة للأحداث

يخاطب الطلبُ رقمَ تسلسل خام واحدًا وأعدادَ جيران اختيارية. وتحمل النتيجةُ `SessionHeader` بدل رايات الإتاحة، فيستطيع هدفٌ حي معروف أن يبقى مستقلًّا عن صحة الحفظ الدائم.
...
تميّز تتبعاتُ الأحداث بين استبدال السطح الموضعي وبين الأحداث المذكورة مصادرَ. وتحتوي كلُّ قائمة أرقام تسلسل روابطَ مباشرة، إلا `replacementChain` التي تتبع المستبدِلين المباشرين من الهدف إلى الاستبدال الموضعي الأخير.
...
يميّز اتحادُ الرموز المغلق بين التحقق من الطلب، والأهدافِ المفقودة، وسجلاتِ السطح المشوَّهة، وفشلِ الخلفية الاختيارية، والبحثِ المعطَّل في النشر، والبياناتِ الوصفية المتناقضة للمصدر.
