...
[SSH مزود بيت عائلة](../../packages/ssh/README.zh.md) عبر نشر جهة يحتفظ OpenSSH اتصال توفير واحد بعيد طرف نظام الملفات/عملية بيئة.Harness، نموذج نقل و Session تخزين إبقاء في رئيسي آلة. هذا بيت عائلة تنفيذ قائم نظام الملفات، عملية فرعية و صندوق رملي API، لا جذب دخول SSH مخصص استخدام نموذج أداة.
...
`processPath()` توفير إعداد طقم عملية فرعية مزود متاح مسار.SSH `processPathFromHostPath()` ما زال غير ممكن استخدام؛ تثبيت بعيد طرف ناتج لا معنى طعم حال مهمة معنى رئيسي آلة مسار يمكن نقل غرس. لذلك [`NodePtcRuntime`](../../packages/ptc-runtime/ptc-runtime-node/README.zh.md) استخدام صريح تثبيت و مرور مرور ملخص تحقق بعيد طرف جذب توجيه برنامج.
...
نشر إقرار إثبات، قد تثبيت ناتج تحقق و تدريجي تدفق TLS إقرار إثبات يخص [`dsh-ssh`](../../packages/ssh/ssh/README.zh.md). مساعد مساعدة عملية استخدام بعيد طرف آلة جهاز فوق يمكن معلومة محلي مزود تنفيذ نظام الملفات و عملية طلب.SSH هو نقل طريقة؛ ملف فاعلية نتيجة حد من الذي اختيار بعيد طرف صندوق رملي مزود تنفيذ.
...
بديل خطة و تحقق مسؤولية مهمة رؤية[قرار سجل](../../.agents/notes/implemented/architecture/2026-09-11-posix-ssh-runtime.zh.md).
