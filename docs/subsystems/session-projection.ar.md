...
seam إسقاط الجلسة — وهو [seam قدرة](../capability-seams.ar.md) تقدّم عبره إضافاتُ المضيف المجالية القيمَ الحالية كاملةً لحالة كل جلسة المشتقة من السجل إلى نواقل العملاء: تعريفُ الخدمة والسجل ([dsh-session-projection](../../packages/session/session-projection)، `ctx.sessionProjections`)، والمساهمون المجاليون (يسجّل كلٌّ منهم وحدةً نقية واحدة)، والنواقل (صفحةُ ذيل التاريخ وإطارُ الدفع `session/projection` في [dsh-session-controller](../../packages/api/session-controller)). وهو قدرةٌ اختيارية واحدة، لا جزءٌ من عمود agent loop. فالإطارُ يقود والمجالُ يحسب: يشترك السجلُّ في `session/event` مرةً واحدة ويطوي كلَّ حدث مودَع عبر كل وحدة؛ ولا تحمل المجالاتُ اشتراكاتٍ ولا يطوي العملاءُ أحداثَ المجال قط — بل يتلقون قيمًا جاهزة. ومرجعُ التصميم هو [RFC إسقاط الجلسة](../../.agents/notes/proposed/architecture/2026-07-27-session-projection-and-command-log.ar.md)؛ وعقودُ القيادة والتخزين والتغذية في [README الحزمة](../../packages/session/session-projection/README.ar.md).

المصدر: [`packages/session/session-projection/src/index.ts`](../../packages/session/session-projection/src/index.ts)

## الوحدة

`SessionProjectionStateMap` هو الجدولُ القابل للتوسعة بالدمج لحالات الطي في المضيف، بينما يحتفظ `SessionProjectionMap` بالقيم الكاملة التي يراها العميل. ويسهم المجالُ بـ`ProjectionDefinition` واحد لكل مفتاح حالة؛ وكتلةُ `wire` تجعل ذلك المفتاحَ مرئيًّا للعميل، أما العرضُ فيخص نظامَ الخانات لا هذه الطبقة:
...
وكلُّ قيمة إسقاط مقدَّمة نموذجُ قراءة كامل. وقد يحمل الحدثُ المصدر قيمةً كاملة أو عمليةً يملكها المجال؛ وتملك `apply` الحتمية في الوحدة إعادةَ التشغيل، وتعيد نقطةُ التفتيش مع إعادة تشغيل الذيل إلى الأمام بناءَ الحالة نفسِها.

## اللقطة وتغذية التغييرات
...
و`snapshot(session)` متزامن بالكامل: يقرؤه الناقلُ في النبضة نفسِها التي يقرأ فيها شريحةَ صفحته، فيغطي `asOfSeq` القراءتين عند رقم تسلسل واحد. وهو لا يعيد إلا عروضَ العميل، وتمرّ كلُّ قيمة عبر `viewSchema` وحدتها قبل الإعادة. ويقرأ `stateOf(session, key)` حالةَ مضيف حية واحدة بلا حساب عروض لا صلةَ لها؛ وعلى المستدعين ألّا يغيّروا المرجعَ المستعار. ويحسب تغيُّرُ مرجع الحالة عرضًا خامًا واحدًا مخزَّنًا، ولا تنطلق تغذيةُ التغييرات إلا حين تتغير تلك النتيجةُ بمقياس `Object.is`؛ وعلى العرض ذي القيمة الكائنية أن يحفظ مرجعَه ليكبح النشرَ عند تغييرات الحالة الداخلية وحدها.

## السجل: `ctx.sessionProjections`

يملك `SessionProjectionRegistry` ([التوقيعات](#ctxsessionprojections--sessionprojectionregistry)) القيادةَ: اشتراكٌ واحد في `session/event`، و`apply` متلهّف على كل وحدة مسجَّلة، وخلايا علامة مائية لكل جلسة ولكل وحدة. وتُبنى الخلايا كسولًا — فالوحدةُ المسجَّلة بعد جريان الأحداث، أو الجلسةُ الأقدم من السجل، تطوي `init` على السجل في الذاكرة عند أول لمسة، حدثًا كانت أو قراءة. والتسجيلُ أثرٌ يركب مُفكِّكُه الليفَ المستدعي: فمفتاحُ إضافة مجالية مفرَّغة، ومعه خلاياها المخزَّنة، يختفي من القيادات واللقطات التالية، ويقرأ العملاءُ ذلك غيابًا للقدرة؛ ويرمي مفتاحٌ مكرر بـ`stateVersion` مختلف، بينما يتشارك المسجّلون بالإصدار نفسِه وحدةً واحدة ويُعدّون. وتسجّل الإضافاتُ المجالية تحت `ctx.inject(['sessionProjections'], …)` فتبقى التجميعاتُ عديمةُ الرأس بلا سجل غيرَ متأثرة.
