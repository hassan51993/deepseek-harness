...
يحمل seam إعدادات المستخدم في [dsh-settings](../../packages/settings/settings) وثيقةً واحدة يملكها المستخدم فيها قسمٌ لكل فضاء أسماء، ويحلّ كلَّ فضاء أسماء مسجَّل بافتراضات الـschema، ثم بطبقة `base` التي يقدّمها التركيبُ لدى المسجِّل، ثم بقسم المستخدم. ويخزّن المزوّدون مثل [dsh-settings-file](../../packages/settings/settings-file) الوثيقةَ الخام ويدفعون التعديلاتِ الخارجية؛ وتسجّل إضافاتُ المستهلكين schema وتقرأ القيمةَ المحلولة أو ترقبها. ويبقى ضبطُ التركيب في `cordis.yml` — فلا يحمل فضاءُ الأسماء إلا الجزءَ القابل لتحرير المستخدم.

المصدر: [`packages/settings/settings/src/index.ts`](../../packages/settings/settings/src/index.ts)

## الهوية

يسمّي فضاءُ الأسماء قسمًا واحدًا من وثيقة المستخدم تملكه إضافةٌ. وتمنع العلامةُ المستدعين من خلط فضاءات أسماء الإعدادات بمعرّفات أخرى تُمرَّر بين الحزم أو العمليات؛ ويتحقق الإنشاءُ من نحو kebab-case بحروف صغيرة.
...
يربط التسجيلُ schema من schemastery بفضاء أسماء على ليف الإضافة المستدعية — فالتخلصُ من ذلك الليف يزيل فضاءَ الأسماء ومراقبيه. وتحمل الخياراتُ طبقةَ التركيب، وتوقيتَ أثر المالك، وفحصًا اختياريًا لما لا تستطيع الـschema التعبيرَ عنه.
...
ويعمل `validate` بعد أن تقبل الـschema قيمةً، فيراها بافتراضاتها وبطبقة التركيب كما سيراها المالكُ بالضبط. وتستعمله `dsh-llm-pi-ai` لترفض ملفَّ مزوّد تعريفيًّا لا تستطيع خدمتَه عند الكتابة التي أنتجته، بدل تخزين ملف يعطّل كلَّ مسار في فضاء أسمائها.

و`applies` تلميحُ واجهة لا آلية: فالمالكُ في وضع `restart` لا يراقب أبدًا، فتُقرأ قيمتُه مرةً واحدة عند الإنشاء وتستطيع واجهاتُ الضبط وسمَ التغيير المعلَّق.
...
النطاقُ هو المقبضُ الذي يراه المالك. و`update` يدمج رقعةً متفرقة فوق قسم المستخدم وحده (لا داخل `base`)؛ و`replace` يضبط القسمَ كاملًا، وهو مسارُ الإزالة وإعادة الضبط — فالمفاتيحُ الغائبة من البديل ترث `base` وافتراضاتِ الـschema من جديد. وتُسلسَل الكتاباتُ إلى فضاء الأسماء الواحد بترتيب ندائها، والقيمُ المحلولة لقطاتٌ مجمَّدة عميقًا.
...
يسلسل `describe()` كلَّ فضاء أسماء مسجَّل لواجهات الضبط: فمغلّفُ `toJSON()` من schemastery يقود النماذجَ المعروضة من الـschema، وتملؤها القيمةُ المحلولة، وتتيح طبقتا `base` و`user` المنفصلتان للنموذج أن يسم الحقولَ التي تجاوزها المستخدمُ بحضورها. ويجرّد `describe({ redactSecrets: true })` — وهو إجباري على كل واجهة شبكية — حقولَ `role('secret')` من الطبقات الثلاث ويعدّد خاناتِها `{path, set}` فتستطيع الصفحةُ عرضَ مُدخَلات للكتابة فقط بلا أن تتلقى سرًّا قط.
...
والمستدعي الذي لا يملك إلا الواصفَ المنقَّح لا يستطيع إعادةَ بناء قسم بأمان، فتسافر الإزالاتُ عملياتِ مسارات بدلًا من ذلك. ويحمل كلُّ واصف أيضًا `revision` فوق القسم الخام؛ وللكتابة أن تعيده `expectedRevision`، وما لم يعد مطابقًا يُرفض بدل أن يُطبَّق فوق الكاتب الذي حطّ أولًا.
```ts type-equiv
/**
...
كلُّ تغيير مودَع — كتابةٌ داخل العملية أو تعديلُ مزوّد رُصد خارجيًا — يُطلق `settings/updated (ns, next, prev, source)` بعد أن تصير القيمةُ الجديدة مرجعًا، ولا يُطلقه قط حين تتساوى القيمةُ المحلولة تساويًا عميقًا. ويفصل وسمُ المصدر بين مسارَي الدخول.
...
## عمليات الوثيقة الأصيلة

تؤكد `SettingsDocumentOpenValue` أن `settings/openSettingsDocument` أعدّ الوثيقةَ التي يملكها المزوّد وسلّمها إلى محرّر النصوص الأصيل. وتبلّغ `AgentPresetDirectoryOpenValue` إما عن تسليم أصيل مكتمل وإما عن دليل presets المستخدم المحلول حين يتعذر الفتحُ على سطح المكتب. ولا تقبل أيٌّ من العمليتين مسارَ مضيف اختاره المتصفح.
