...
عند كثير بند ذاك هذا مختلف Every سجل متساو قد إلى مدة، كما لا يوجد مرة صفة رفع تنبيه إلى مدة وقت، كل بند سجل كل سوف نحو نفس عدد follow-up دفعة مرة مساهمة مرة إطلاق، و حسب هدف وقت و إنشاء ترتيب صف. كل بند Every سجل حالة متبادل متبادل مستقل، لكن هذا نيل دقيق دفعة مرة في كل dispatch كل استخدام نفس عدد حكم قطع وقت لحظة. دفعة معالجة حد نموذج جولة عدد كمية؛ خمسة دقيقة تحت حد حد كل بند سجل timer تردد معدل.
