...
Webhook فرعي نظام سوف يأخذ قد عبر هوية تحقق خارجي تسليم تحويل لـ اختياري عادي أصل Session. مزود مهايئ يملك هوية تحقق و عام JSON استقبال؛ تلقي معلومة مهمة برنامج تحويل قاعدة يملك شرط و خارجي استدعاء؛`ctx.webhookRuntime` يملك عودة ضبط دورة الحياة و أساس في Workspace Session إنشاء.[قد تنفيذ قرار](../../.agents/notes/implemented/feature/2026-08-22-fire-and-forget-webhook-sessions.ar.md) سجل runtime لـ أي لا إبقاء تسليم أو إتمام حالة.
...
`dispatch()` سوف لقطة مطابقة قاعدة، ذاك هذا مستقل أرض ضبط درجة كل قاعدة، و في أي عودة ضبط تسوية قبل إرجاع. رمي خروج و رفض حسب قاعدة قسم آخر يتم يتضمن. تسجيل disposer سوف أولا إزالة قاعدة، مجددا في توقف و ترتيب فارغ نشط حركة استدعاء، لذلك لاحق تسليم لا يمكن دخول جارٍ إزالة شفرة.
...
[GitHub مراجعة إشارة جنوب](../user/guide/github-review.ar.md) يأخذ هذا توجيه تركيب في عزل ثاني عدد WebServer فوق، لذلك كشف webhook مدخل لن كشف متصفح API.
