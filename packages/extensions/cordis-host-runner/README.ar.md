...
|---|---|---|
| `vmTimeoutMs` | `5000` | host نصف في vm في تزامن تنفيذ ذلك جزء يتم في توقف طلب قيمة قبل يمكن تشغيل مللي ثانية عدد |

توليد[إعداد دليل](../../../docs/config-catalog.ar.md#deepseek-aidsh-cordis-host-runner) هو كل تلقي دعم حمل حقل نفاد كل صيغة حق مصدر.
...
صندوق رملي عزل عام متغير، لكن لا هو أمان حد:Node عام متغير لا وجود، أو إعادة تحديد نحو إلى Cordis خدمة (`ctx.fs`،`ctx.web`،`ctx.bash` و تحديد وقت جهاز helper) ،host نصف استلام إلى هو لا يحتوي إطار هيكل داخلي آلية façade، لكن هو إعلان خدمة ما زال سوف لمس بلوغ تخزين نشط وقت التشغيل. مقابل انتظار حركة حالة حزمة يلزم مثل مقابل انتظار bash وصول واحد مثال، مشاركة رؤية[ذاتي مرجع أداة تجميع Agent Note](../../../.agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.ar.md).
...
- [أداة حزمة](../tool-cordis/README.ar.md)——استخدام ذلك فحص سجل التسجيل فقط قراءة أداة.
- [Client runner](../cordis-client-runner/README.ar.md)——ينبغي جواب تشغيل طلب و تركيب تحميل متصفح نصف شفرة متصفح نصف.
- [UI حزمة](../ui-cordis/README.ar.md)——مستخدم دفعة دقيق و عملية تشغيل وجه لوح.
- [توليد إعداد دليل](../../../docs/config-catalog.ar.md#deepseek-aidsh-cordis-host-runner)——كل تلقي دعم حمل إعداد حقل.
- [extensions فرعي نظام](../../../docs/subsystems/extensions.ar.md)——توليد `ctx.cordisInspect` و `ctx.dynamicCordisRunner` API و `cordis/*` حدث.
- [ذاتي مرجع Cordis أداة تجميع Agent Note](../../../.agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.ar.md)——صندوق رملي دلالة، دورة الحياة و تركيب إدارة من.
