...
const ARGS = '{"name":"clock","purpose":"قمة شريط وقت ساعة","code":{"client":"return {}","host":"harness.handle(\'now\', () => Date.now())"}}'
...
describe('cordisDefineCard', () => {
  it('reads name, purpose and both code halves off the call arguments', () => {
    ...
    expect(card).toMatchObject({
      name: 'clock', purpose: 'قمة شريط وقت ساعة', clientCode: 'return {}', state: 'running', output: null,
    })
  ...
  it('keeps the raw first line as the name when the arguments carry none', () => {
    expect(cordisDefineCard(running({ argsRaw: '{"purpose":"قمة شريط وقت ساعة"}' })).name).toBe('{"purpose":"قمة شريط وقت ساعة"}')
    expect(cordisDefineCard(running({ argsRaw: '{"name":"","purpose":"قمة شريط وقت ساعة"}' })).name).toBe('{"name":"","purpose":"قمة شريط وقت ساعة"}')
  })
