...
describe('loadWin32DialogBindings over the fake COM world', () => {
  ...
  it.each([
    { label: 'zero-low-byte BMP code units', text: 'C:/fixture/أمان بارز تطوير' },
    { label: 'surrogate pairs', text: 'C:/fixture/😀' },
    { label: 'more than 32 KiB', text: 'C:/' + 'فتح'.repeat(17_000) },
    { label: 'NUL termination', text: 'C:/فتح' + String.fromCharCode(0) + 'ignored' },
  ...
  it('cascades DPI contexts to the first the host accepts', async () => {
    ...
    installFakeKoffi(world)
    const bindings = await (await loadBindingsModule()).loadWin32DialogBindings()
    expect(runFolderDialog(bindings, 'Pick', vi.fn())).toBe('C:\\اختيار في\\directory')
