...
`host` فقط قبول اثنان عدد قيمة:`127.0.0.1`(افتراضي وضع حالة، فقط عودة حلقة) و `0.0.0.0`(متعمد نحو شبكة فتح وضع——خادم ذاته لا يحمل TLS، إقرار إثبات أو مصدر سياسة).`port` لـ 0 وقت طلب OS قسم إعداد طرف فتحة؛ بعد استخدام `ctx.webServer.port` قراءة جارٍ استماع طرف فتحة.
...
- **لا توفير خادم درجة TLS، إقرار إثبات أو مصدر سياسة**:`dsh-client-connection` انتظار route owner سوف فعلي تطبيق ذاتي ذات طلب سياسة. ربط غير عودة حلقة عنوان ما زال سوف نحو هذا شبكة عام لم تلقي حفظ حماية route و ساكن حالة مورد.
