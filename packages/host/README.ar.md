...
`host/` مجموعة توفير Web GUI عادي HTTP خادم، خدمة قد بناء Web قشرة SPA dist خادم، حمل أصلي/تصفح/ذاتي ملائم ينبغي تركيب حزمة مساحة العمل دليل اختيار seam،open-in-app تطبيق استكشاف قياس و بدء توجيه، و فقط قراءة إضافة بيان إسقاط. هذا ثمانية عدد حزمة كل هو منتج حزمة؛ متصفح نقل يقع في [`client/`](../client/README.ar.md) ، تركيب تطبيق هو [`apps/cli`](../../apps/cli/README.ar.md) ، هو بدء [`dsh-base` تركيب حزمة](../bundle/base/cordis.patch.yml) قدوم توفير `apps/web/` تحت Web تطبيق. اختيار جهاز خلفية يمكن في مشترك seam بعد متبادل متبادل استبدال.
