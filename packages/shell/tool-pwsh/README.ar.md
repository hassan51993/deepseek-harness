...
| [`src/render.ts`](src/render.ts) | نموذج جانب نتيجة نص: تدفق، علامة، مقتطع إشعار (bash توأم توليد) |
...
خلفية بدء دقيق تصيير لـ `started background job <id>`؛ مع بعد قراءة و حالة مرور من عام `job_output`/`job_kill` أداة تدفق تحويل، يشمل داخل تخزين مقتطع إسقاط لم قراءة بايت وقت لديه ضرر قراءة spill إشعار.
