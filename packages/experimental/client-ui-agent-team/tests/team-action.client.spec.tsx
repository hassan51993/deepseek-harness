...
describe('TeamAction', () => {
  ...
  it('keeps a successful task mutation newer than an in-flight refresh', async () => {
    ...
    expect(await screen.findByRole('button', { name: /إعادة الفتح/u })).toBeTruthy()

    stale.resolve({ ok: true, value: view })
    await Promise.resolve()
    expect(screen.getByRole('button', { name: /إعادة الفتح/u })).toBeTruthy()
  ...
  it('keeps a created task newer than an in-flight refresh', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: ar.refresh }))
    fireEvent.click(screen.getByRole('button', { name: /المهام جديدة/u }))
    fireEvent.change(screen.getByPlaceholderText('عنوان الالمهام'), { target: { value: 'New task' } })
    fireEvent.change(screen.getByPlaceholderText('وصف الالمهام'), { target: { value: 'Details' } })
  ...
  it('keeps task and create failures newer than an in-flight refresh', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: ar.refresh }))
    fireEvent.click(screen.getByRole('button', { name: /المهام جديدة/u }))
    fireEvent.change(screen.getByPlaceholderText('عنوان الالمهام'), { target: { value: 'Rejected task' } })
    fireEvent.change(screen.getByPlaceholderText('وصف الالمهام'), { target: { value: 'Rejected details' } })
    fireEvent.click(screen.getByRole('button', { name: 'حفظ' }))
  ...
  it('tracks simultaneous create and task mutations independently', async () => {
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /المهام جديدة/u }))
    fireEvent.change(screen.getByPlaceholderText('عنوان الالمهام'), { target: { value: 'Concurrent task' } })
    fireEvent.change(screen.getByPlaceholderText('وصف الالمهام'), { target: { value: 'Concurrent details' } })
  ...
  it('creates a task from normalized blocker and write-scope lists', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('عنوان الالمهام'), { target: { value: ' New task ' } })
    fireEvent.change(screen.getByPlaceholderText('وصف الالمهام'), { target: { value: ' Details ' } })
    fireEvent.change(screen.getByPlaceholderText(/معطَّلة بـ المهام/u), { target: { value: 'task-1, task-1' } })
  ...
  it('assigns, edits, completes, reopens, and deletes with contiguous CAS revisions', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('عنوان الالمهام'), { target: { value: 'Updated runtime' } })
    fireEvent.change(screen.getByPlaceholderText('وصف الالمهام'), { target: { value: 'Updated details' } })
    fireEvent.change(screen.getByPlaceholderText(/معطَّلة بـ المهام/u), { target: { value: 'task-0' } })
    ...
    fireEvent.click(screen.getByRole('button', { name: /إتمام/u }))
    fireEvent.click(await screen.findByRole('button', { name: /إعادة الفتح/u }))
    await waitFor(() => {
      expect(screen.queryByRole('button', { name: /إعادة الفتح/u })).toBeNull()
  ...
  it('keeps reload failures visible after task and dependency conflicts', async () => {
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /تحرير/u }))
    fireEvent.change(screen.getByPlaceholderText('عنوان الالمهام'), { target: { value: 'Edited' } })
  ...
  it('shows load and create failures and ignores a create result after a session switch', async () => {
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /المهام جديدة/u }))
    fireEvent.change(screen.getByPlaceholderText('عنوان الالمهام'), { target: { value: 'Task' } })
    fireEvent.change(screen.getByPlaceholderText('وصف الالمهام'), { target: { value: 'Description' } })
    fireEvent.click(screen.getByRole('button', { name: 'حفظ' }))
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /المهام جديدة/u }))
    fireEvent.change(screen.getByPlaceholderText('عنوان الالمهام'), { target: { value: 'Late task' } })
    fireEvent.change(screen.getByPlaceholderText('وصف الالمهام'), { target: { value: 'Late description' } })
    fireEvent.click(screen.getByRole('button', { name: 'حفظ' }))
  ...
  it('contains edit and dependency failures and supports form cancellation and unassignment', async () => {
    ...
    expect(screen.queryByPlaceholderText('عنوان الالمهام')).toBeNull()
    ...
    fireEvent.change(screen.getByPlaceholderText('عنوان الالمهام'), { target: { value: 'Saved edit' } })
    fireEvent.change(screen.getByPlaceholderText(ar.blockers), { target: { value: 'task-2' } })
    fireEvent.click(screen.getByRole('button', { name: 'حفظ' }))
  ...
  it('skips the dependency mutation when an edit keeps the same blockers', async () => {
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /تحرير/u }))
    fireEvent.change(screen.getByPlaceholderText('عنوان الالمهام'), { target: { value: 'Same dependencies' } })
    fireEvent.click(screen.getByRole('button', { name: 'حفظ' }))
  ...
  it('reloads a dependency conflict and ignores dependency settlement after a session switch', async () => {
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /تحرير/u }))
    fireEvent.change(screen.getByPlaceholderText('عنوان الالمهام'), { target: { value: 'Conflict edit' } })
    fireEvent.change(screen.getByPlaceholderText(ar.blockers), { target: { value: 'task-2' } })
    fireEvent.click(screen.getByRole('button', { name: 'حفظ' }))
    expect(await screen.findByText(ar.conflict)).toBeTruthy()
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /تحرير/u }))
    fireEvent.change(screen.getByPlaceholderText('عنوان الالمهام'), { target: { value: 'Late edit' } })
    fireEvent.change(screen.getByPlaceholderText(ar.blockers), { target: { value: 'task-2' } })
    fireEvent.click(screen.getByRole('button', { name: 'حفظ' }))
