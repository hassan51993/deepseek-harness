...
describe('TeamAction', () => {
  ...
  it('keeps a successful task mutation newer than an in-flight refresh', async () => {
    ...
    expect(screen.queryByRole('button', { name: /إنهاء/u })).toBeNull()
  })
  ...
  it('keeps task and create failures newer than an in-flight refresh', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: ar.refresh }))
    fireEvent.click(screen.getByRole('button', { name: /مهمة جديدة/u }))
    fireEvent.change(screen.getByPlaceholderText('عنوان المهمة'), { target: { value: 'Rejected task' } })
    fireEvent.change(screen.getByPlaceholderText('وصف المهمة'), { target: { value: 'Rejected details' } })
    fireEvent.click(screen.getByRole('button', { name: 'حفظ' }))
  ...
  it('tracks simultaneous create and task mutations independently', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('وصف المهمة'), { target: { value: 'Concurrent details' } })
    const save = screen.getByRole<HTMLButtonElement>('button', { name: 'حفظ' })
    fireEvent.click(save)
    await waitFor(() => { expect(save.disabled).toBe(true) })

    const complete = screen.getByRole<HTMLButtonElement>('button', { name: /إنهاء/u })
  ...
  it('shows load and create failures and ignores a create result after a session switch', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('وصف المهمة'), { target: { value: 'Description' } })
    fireEvent.click(screen.getByRole('button', { name: 'حفظ' }))
    ...
    fireEvent.change(screen.getByPlaceholderText('وصف المهمة'), { target: { value: 'Late description' } })
    fireEvent.click(screen.getByRole('button', { name: 'حفظ' }))
  ...
  it('contains edit and dependency failures and supports form cancellation and unassignment', async () => {
    ...
    expect(screen.queryByPlaceholderText('عنوان المهمة')).toBeNull()
    ...
    fireEvent.change(screen.getByPlaceholderText('عنوان المهمة'), { target: { value: 'Saved edit' } })
    fireEvent.change(screen.getByPlaceholderText(ar.blockers), { target: { value: 'task-2' } })
    fireEvent.click(screen.getByRole('button', { name: 'حفظ' }))
