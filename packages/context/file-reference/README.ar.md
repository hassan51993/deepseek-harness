...
مضيف قيادة UI استخدام `dsh-file-reference` توفير `@file` تكملة كل:UI لـ إشارة تحديد agent(ذكي جسم) طلب مسار مرشح، نموذج إدخال `@path` أو `@"path with spaces"`، اختيار في مرشح بعد، مطابقة mention بصفة عادي نص التوجيه نص إدراج دخول.seam ذاته لا يملك نظام الملفات وصول——أداة جسم مزود (مثل `@deepseek-ai/dsh-file-reference-local`) مسؤول توفير مرشح، ترتيب، ذاكرة مؤقتة و بطلان. اختيار في مرشح أبدا قراءة أو مرفق حمل ملف محتوى؛ نموذج يجب استدعاء نظام الملفات أداة عندئذ قدرة عرض ملف.Session Controller عبر `fileReferences/list` Remote نحو متصفح مستهلك كشف نفس اكتشاف قدرة.
...
`ctx.fileReferences.list(agent, query, signal)` إرجاع إشارة تحديد agent عمل دليل في فقط يحتوي مسار ملف و دليل مرشح، من مزود تحديد صفة أرض ترتيب. دليل mention عرض وقت حمل ذيل مع `/`، جعل تكملة كل يمكن متابعة عميق دخول تحت واحد طبقة. متصفح مستهلك عبر Session Controller مهايئ `ctx.remote.fileReferences.list` استدعاء نفس اكتشاف قدرة؛ نهاية موضع signal معامل يمكن إلغاء بطيء سرعة تلقائي تكملة كل.
...
هذه الحزمة يأخذ سحب كائن اكتشاف خدمة و مشترك، متصفح أمان mention لغة قاعدة قسم فتح، من مزود مسؤول نطاق الأسماء وصول، ترتيب، ذاكرة مؤقتة و بطلان. هذا خدمة إبقاء wire في قيام؛`dsh-api-session-controller` يحتفظ `fileReferences/list` Remote مهايئ، و في تحليل Agent بعد تفويض إرسال إعطاء حالي مزود.
...
- [محلي ملف مرجع مزود](../file-reference-local/README.ar.md)——هذا seam مع مرفق محلي مساحة العمل تنفيذ.
- [جلسة مرجع فرعي نظام](../../../docs/subsystems/session-reference.ar.md)——مضيف UI خلف بعد مشترك ملف مرجع و جلسة مرجع اتفاق.
- [سياق مجموعة أرض رسم](../README.ar.md)——متبادل مجاور طلب سياق حزمة.
- [نظام الملفات أداة دليل](../../../docs/tool-catalog.ar.md#deepseek-aidsh-tool-fs)——يتم مرجع مسار الذي مقابل `read` أداة.
