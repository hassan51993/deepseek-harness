...
describe('MessageItem arms', () => {
  it('renders an adjacent session mention as a chip even without trailing whitespace', () => {
    const view = render(
      <MessageItem
        t={t}
        referenceLabels={['أنت جيد']}
        node={{
          kind: 'user',
          seq: 1,
          time: 1_000,
          content: [{ type: 'text', text: '@أنت جيد هذا عدد في شرح ماذا' }] as never,
          source: null,
        }}
      />,
    )
    expect(view.container.querySelector('[data-ref-chip="session"]')?.textContent).toBe('أنت جيد')
    expect(view.container.querySelector('[data-ref-chip="session"] svg')).not.toBeNull()
    expect(view.getByText('هذا عدد في شرح ماذا')).toBeTruthy()
    expect(view.getByText('مرجع الجلسات · أنت جيد')).toBeTruthy()
  })
  ...
  it('renders the complete metadata-confirmed multi-word session label', () => {
    ...
    expect(view.getByText('مرجع الجلسات · Research notes')).toBeTruthy()
  })
  ...
  it('user bubbles expose clock / copy and neither branch nor edit; copy writes the text', () => {
    ...
    expect(screen.queryByRole('button', { name: 'التفرّع إلى الجلسات جديدة' })).toBeNull()
    expect(screen.queryByRole('button', { name: 'تحرير' })).toBeNull()
  ...
  it('copy swaps to the check success chrome, gates re-clicks, and reverts after a second', async () => {
    ...
    const done = screen.getByRole('button', { name: 'تم النسخ' })
  ...
  it('consumed steering renders as a plain user bubble and keeps copy without branch', () => {
    ...
    expect(view.queryByText('إدراج كلام')).toBeNull()
    expect(view.getByText('steer!')).toBeTruthy()
    expect(view.getByText(/كتلة المحتوى إضافية/)).toBeTruthy()
    fireEvent.click(view.getByRole('button', { name: 'نسخ' }))
    expect(writeText).toHaveBeenCalledWith('steer!')
    expect(view.queryByRole('button', { name: 'التفرّع إلى الجلسات جديدة' })).toBeNull()
  })
  ...
  it('context uses the Tool calls disclosure chrome and keeps its body collapsed by default', () => {
    ...
    const disclosure = ctxView.getByRole('button', { name: /^حقن الالسياق\s*fixture$/ })
  ...
  it('the instructions form names the files it reconciled above their text', () => {
    ...
    fireEvent.click(view.getByRole('button', { name: /^حقن الالسياق\s*AGENTS\.md, sub\/AGENTS\.md$/ }))
    const files = [...view.container.querySelectorAll('[data-context-files] li')].map(node => node.textContent)
    expect(files).toEqual(['AGENTS.mdحُمِّلت', 'sub/AGENTS.mdأُزيلت'])
  ...
  it('a delta distinguishes a newly reconciled file from a rewritten one', () => {
    ...
    fireEvent.click(view.getByRole('button', { name: /^حقن الالسياق\s*new\/AGENTS\.md, old\/AGENTS\.md$/ }))
    const files = [...view.container.querySelectorAll('[data-context-files] li')].map(node => node.textContent)
  ...
  it('keeps an interleaved unknown block in the order the model received it', () => {
    ...
    fireEvent.click(view.getByRole('button', { name: 'حقن الالسياق' }))
  ...
  it('the catalog form lists its durable entries instead of the model-facing prose', () => {
    ...
    fireEvent.click(view.getByRole('button', { name: /^حقن الالسياق\s*skill-catalog$/ }))
  ...
  it('a replacement catalog says so above its entries', () => {
    ...
    fireEvent.click(view.getByRole('button', { name: /^حقن الالسياق\s*skill-catalog$/ }))
    expect(view.container.querySelector('[data-context-catalog-update]')?.textContent).toBe('كتالوج بديل')
  })
  ...
  it('an unreadable instruction list falls back to the opaque body with its fields', () => {
    ...
    fireEvent.click(view.getByRole('button', { name: /^حقن الالسياق\s*agent-instructions$/ }))
  ...
  it('bounds an oversized source field, not only the model-facing text', () => {
    ...
    fireEvent.click(view.getByRole('button', { name: /^حقن الالسياق\s*plugin$/ }))
    expect(view.container.querySelector('[data-context-fields] dd')?.textContent)
      .toMatch(/… مقتطع، مشترك \d+ محرف$/)
  })
  ...
  it('bounds a large catalog and says how many rows it withheld', () => {
    ...
    expect(view.container.querySelector('[data-context-entries-truncated]')?.textContent).toBe('…أيضا لديه 5 بند')
  })
  ...
  it('the opaque fallback keeps a form declaration this version cannot present', () => {
    ...
    fireEvent.click(view.getByRole('button', { name: /^حقن الالسياق\s*later$/ }))
  ...
  it('the snapshot form attributes each part to the subsystem that produced it', () => {
    ...
    fireEvent.click(view.getByRole('button', { name: /^حقن الالسياق\s*@deepseek-ai\/dsh-system-prompt$/ }))
  ...
  it('a notice without its account falls back to the opaque body', () => {
    ...
    fireEvent.click(view.getByRole('button', { name: /^حقن الالسياق\s*tool-jobs$/ }))
    expect(view.container.querySelector('[data-context-fields]')).not.toBeNull()
  })
  ...
  it('each form falls back to the opaque body when its required facts are unreadable', () => {
    ...
    for (const { form, source, label } of cases) {
      ...
      fireEvent.click(view.getByRole('button', { name: new RegExp(`^حقن الالسياق\\s*${label}$`) }))
  ...
  it('a relay names the agent that sent it above what it said', () => {
    ...
    fireEvent.click(view.getByRole('button', { name: /^حقن الالسياق\s*agent-message$/ }))
    expect(view.container.querySelector('[data-context-relay-sender]')?.textContent).toBe('من الجلسات child-7')
    ...
    const view = render(
      <MessageItem t={t} node={{
        ...
        source: {
          ...
          version: 1,
          references: [
            { label: 'إعادة بنية loader', retainedMessages: 18, omittedMessages: 42, truncated: true },
            { label: 'إصلاح CI', retainedMessages: 3, omittedMessages: 0, truncated: false },
          ],
        },
        producer: { role: 'recall', label: 'إعادة بنية loader, إصلاح CI' },
        form: 'recall',
      } as never}
      />,
    )
    expect(view.container.querySelector('[data-context-recall-icon]')).not.toBeNull()
    fireEvent.click(view.getByRole('button', { name: /^استرجاع من الجلسات\s*إعادة بنية loader, إصلاح CI$/ }))
    const rows = [...view.container.querySelectorAll('[data-context-recalls] li')].map(node => node.textContent)
    expect(rows).toEqual(['إعادة بنية loaderإبقاء 18 بند · حذف 42 بندمقتطع', 'إصلاح CIإبقاء 3 بند · حذف 0 بند'])
  ...
  it('unknown nodes retain the generic JSON row', () => {
    ...
    expect(unknownView.getByText(/غير معروف surface حدث:surface\/next/)).toBeTruthy()
  })
  ...
  it('a compaction marker discloses its summary and never shows the framed checkpoint', () => {
    ...
    const row = view.getByRole('button', { name: /ضُغط الالسياق/ })
    ...
    expect(view.getByRole('heading', { name: 'ملخص عنوان' })).toBeTruthy()
    fireEvent.click(row)
    expect(row.getAttribute('aria-expanded')).toBe('false')
  })
  ...
  it('anchors the sticky-header selector: the compaction body sits under compactionRow only while open', () => {
    ...
    fireEvent.click(view.getByRole('button', { name: /ضُغط الالسياق/ }))
  ...
  it('a marker whose cited summary event fell outside the window is not expandable', () => {
    ...
    expect(view.getByText('ملخّص الضغط غير متاح')).toBeTruthy()
  ...
  it('collapses retry details behind the durable model retry status', () => {
    ...
    expect(view.getByRole('status').textContent).toBe('جارٍ إعادة المحاولة طلب الالنموذج (1/2) · 3s')
    expect(view.getByText('مهلة إعادة المحاولة:').parentElement?.textContent).toBe('مهلة إعادة المحاولة: 2500مللي ثوانٍ')
    expect(view.getByText('سبب الفشل:').parentElement?.textContent).toBe('سبب الفشل: اتصال يتم إعادة وضع')

    act(() => { vi.advanceTimersByTime(1_100) })
    expect(view.getByRole('status').textContent).toBe('جارٍ إعادة المحاولة طلب الالنموذج (1/2) · 2s')
    act(() => { vi.advanceTimersByTime(1_000) })
    expect(view.getByRole('status').textContent).toBe('جارٍ إعادة المحاولة طلب الالنموذج (1/2) · 1s')
    ...
    view.rerender(
      <MessageItem
        ...
        node={{
          ...
          maxRetries: 2,
          delayMs: 3_500.4,
          failure: { code: 'TRANSPORT', message: 'مجددا مرة قطع فتح' },
        }}
      />,
    )
    expect(view.getByRole('status').textContent).toBe('جارٍ إعادة المحاولة طلب الالنموذج (2/2) · 4s')
    ...
    expect(view.getByRole('status').textContent).toBe('أُعيدت محاولة طلب الالنموذج (2/2) · 4s')
    ...
    view.rerender(
      <MessageItem t={t} node={{
        ...
        retry: 3,
        delayMs: 3_500.4,
        failure: { code: 'TRANSPORT', message: 'متابعة إعادة المحاولة' },
      }}
      />,
    )
    expect(view.getByRole('status').textContent).toBe('أُعيدت محاولة طلب الالنموذج (3/∞) · 4s')
    ...
    view.rerender(
      <MessageItem t={t} node={{
        ...
        maxRetries: 2,
        delayMs: 3_500.4,
        failure: { code: 'TRANSPORT', message: 'المستخدم إلغاء' },
      }}
      />,
    )
    expect(view.getByRole('status').textContent).toBe('أُلغيت إعادة المحاولة طلب الالنموذج (1/2) · 4s')
  })
...
describe('formatMessageClock', () => {
  ...
  it('prefixes month and day across days in the same year', () => {
    expect(formatMessageClock(new Date(2026, 0, 1, 14, 24).getTime(), t, now)).toBe('1شهر1يوم 14:24')
  })

  it('prefixes year, month, and day across years', () => {
    expect(formatMessageClock(new Date(2025, 11, 31, 9, 5).getTime(), t, now)).toBe('2025سنة12شهر31يوم 09:05')
  })
...
describe('useCalendarDay boundary refresh', () => {
  ...
  it('widens a same-day user clock after local midnight', () => {
    ...
    expect(screen.getByText('7شهر29يوم 14:24')).toBeTruthy()
  })
})
...
describe('small branch tails', () => {
  ...
  it('StatsPills omits the cache-hit segment when no input accounting exists at all', () => {
    ...
    expect(view.getByText('1 الجولات 1 خطوة').closest('button')).toBeNull()
