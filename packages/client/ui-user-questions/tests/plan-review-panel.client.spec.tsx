...
describe('PlanReviewPanel', () => {
  ...
  it('shows the plan title and summary above two review actions', () => {
    ...
    expect(screen.getByText(ar['plan.header'])).toBeTruthy()
    ...
    expect(screen.queryByText(ar['action.skip'])).toBeNull()
  ...
  it('answers with the asker\'s approve label and keeps its description as the tooltip', () => {
    ...
    const approve = screen.getByRole('button', { name: ar['plan.approve'] })
    ...
    expect(screen.getByRole('button', { name: ar['plan.discuss'] }).hasAttribute('disabled')).toBe(true)
    fireEvent.click(approve)
  ...
  it('dismisses the request so the composer returns for a plain message', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: ar['plan.discuss'] }))
  ...
  it('omits the tooltip for an option carrying no description', () => {
    ...
    expect(screen.getByRole('button', { name: ar['plan.approve'] }).hasAttribute('title')).toBe(false)
  })
  ...
  it('hides the decline action when the asker offered approve alone', () => {
    ...
    expect(screen.queryByRole('button', { name: ar['plan.decline'] })).toBeNull()
    expect(screen.getByRole('button', { name: ar['plan.approve'] })).toBeTruthy()
  })
  ...
  it('re-arms the actions and says why when the decision does not land', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: ar['plan.approve'] }))
    ...
    expect(screen.getByRole('button', { name: ar['plan.approve'] }).hasAttribute('disabled')).toBe(false)
    fireEvent.click(screen.getByRole('button', { name: ar['plan.approve'] }))
