...
import { en, NS, ar } from '../src/client/locales.ts'
...
async function bench(): Promise<{ ctx: Context; fiber: ReturnType<Context['plugin']> }> {
  ...
  // These specs assert the shipped Arabic copy. There is no jsdom `window` in
...
describe('ui-job browser half', () => {
  ...
  it('registers both dictionaries under its own namespace and releases them with the fiber', async () => {
    ...
    expect(translate('list.aria')).toBe(ar['list.aria'])
  ...
  it('keeps the English dictionary key-identical to the Arabic source of truth', () => {
