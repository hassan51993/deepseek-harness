...
Each durable `/goal` run projects as a right-aligned user-style bubble labeled `Command input` (or `إشارة أمر إدخال`), rendered before the generic command result row; the leading `/goal` token renders as a command reference chip in the code face through ui-primitives `projectUserText`, and the objective stays plain body text. It carries no timestamp, copy, or branch actions, and reloading reconstructs it from the run.
