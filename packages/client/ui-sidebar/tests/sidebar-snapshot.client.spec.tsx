 ...
 * the service's default locale (ar — the fallback chain's base), pinning
...
async function bench(options: { locale?: 'en' } = {}) {
  ...
  locale.register('common', { ar: commonAr, en: commonEn })
...
describe('sidebar shell snapshots', () => {
  it('renders the expanded column in the default locale (ar, no setLocale)', async () => {
    ...
    expect(slot.view.getAllByRole('button', { name: 'جديد بناء جلسة' })).toHaveLength(2)
  ...
  it('a locale switch refreshes mounted copy without re-registration', async () => {
    ...
    expect(slot.view.getAllByRole('button', { name: 'New session' })).toHaveLength(2)
    expect(slot.view.queryByRole('button', { name: 'جديد بناء جلسة' })).toBeNull()
    await runtime.dispose()
  })
