...
import { en, ar } from '../src/client/code/locales.ts'
...
describe('CodeBody', () => {
  ...
  it('passes localized controls and ignores byte contents outside its loading mode', () => {
    const view = render(<CodeBody {...props(contents(['const a = 1'], true), { t: key => ar[key as keyof typeof ar] })} />)
    expect(view.getByRole('button', { name: 'نسخ' })).toBeTruthy()
