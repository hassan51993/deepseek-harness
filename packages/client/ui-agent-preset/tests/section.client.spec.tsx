...
const READY: AgentPresetSectionState = {
  ...
  policySaving: false,
  rows: [
    { id: 'standard', trust: 'system', isDefault: true, name: 'النمط العاديي', description: 'كامل تحرير رمز agent.' },
...
describe('the preset list', () => {
  ...
  it('withholds the viewer on a broken shipped preset', () => {
    renderSection({
      rows: [{ id: 'standard', trust: 'system', isDefault: false, name: 'النمط العاديي', broken: 'the composition is not valid YAML' }],
    })
  ...
  it('keeps the empty custom group on screen: heading plus the creator entry', () => {
    renderSection({
      rows: [
        { id: 'standard', trust: 'system', isDefault: true, name: 'النمط العاديي' },
        { id: 'cordis', trust: 'system', isDefault: false, name: 'إنشاء صنع نمط' },
      ],
    })
...
describe('the read-only viewer', () => {
  it('shows the composition text under the preset\'s name', () => {
    renderSection({ view: { id: 'standard', title: 'النمط العاديي', content: '- id: tool-bash\n' } })
  ...
  it('closes through the controller', () => {
    const actions = renderSection({ view: { id: 'standard', title: 'النمط العاديي', content: '- id: x\n' } })
...
describe('a long card description', () => {
  ...
  const LONG = 'بداية نهاية استخدام بسيط جسم العربية تسليم تدفق صديق جيد عام المساعد، توفير حمل دائم bash و ملف تحرير قدرة.'.repeat(8)
  ...
  it('offers the whole description on hover once the card cuts it off', () => {
    ...
    vi.useFakeTimers()
    try {
      renderSection({ rows: [{ id: 'ar', trust: 'user', isDefault: false, name: 'العربية المساعد', description: LONG }] })

      fireEvent.mouseEnter(within(rowFor('ar')).getByText(LONG))
  ...
  it('stays quiet when the description already fits', () => {
    ...
    vi.useFakeTimers()
    try {
      renderSection({ rows: [{ id: 'ar', trust: 'user', isDefault: false, name: 'العربية المساعد', description: 'قصير وصف.' }] })

      fireEvent.mouseEnter(within(rowFor('ar')).getByText('قصير وصف.'))
      act(() => { vi.advanceTimersByTime(400) })
  ...
  it('renders where the runtime has no ResizeObserver', () => {
    ...
    expect(() => {
      renderSection({ rows: [{ id: 'ar', trust: 'user', isDefault: false, description: LONG }] })
    }).not.toThrow()
    // The first measurement does not depend on the observer.
    expect(within(rowFor('ar')).getByText(LONG).getAttribute('title')).toBe('')
  })
})
