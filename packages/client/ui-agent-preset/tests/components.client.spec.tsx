...
const ROSTER_READY: AgentPresetSettingsState = {
  status: 'ready',
  error: null,
  options: [{ id: 'standard', trust: 'system', name: 'معيار نمط' }, { id: 'mine', trust: 'user' }],
}
...
const SEAT_READY: AgentPresetSeatState = {
  ...
  current: 'standard',
  options: [
    { id: 'standard', trust: 'system', name: 'معيار نمط', description: 'كامل تحرير رمز agent.' },
    { id: 'mine', trust: 'user' },
  ],
  busy: false,
  error: null,
...
describe('the chip introduce cue', () => {
  ...
  it('keeps the per-tick cap for a short CJK name', () => {
    ...
    renderSeat({
      current: 'creator',
      options: [{ id: 'creator', trust: 'user', name: 'إنشاء صنع نمط' }],
      introduce: true,
    })
