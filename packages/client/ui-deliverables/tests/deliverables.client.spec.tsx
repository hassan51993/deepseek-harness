...
describe('ChangedFiles card', () => {
  ...
  it('opens the review the same way without a desktop', () => {
    ...
    expect(view.getByRole('button', { name: 'مراجعة تغييرات هذه الجولة في الشريط الجانبي' })).toBeTruthy()
    ...
    expect(openFile).not.toHaveBeenCalled()
    expect(props.openChanged).not.toHaveBeenCalled()
    expect(view.getByRole('button', { name: 'عرض كل الملفات المتغيّرة (5)' }).textContent).toContain('كل الملفات (5)')
  })
