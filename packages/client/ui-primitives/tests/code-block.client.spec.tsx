...
describe('CodeBlock', () => {
  ...
  it.each(['ts', 'unregistered'])('numbers %s source lines without copying the gutter', async (lang) => {
    ...
    try {
      ...
      await act(async () => { fireEvent.click(view.getByRole('button', { name: 'نسخ' })) })
  ...
  it('does not claim success when execCommand throws or is absent', async () => {
    ...
    fireEvent.click(denied.getByRole('button', { name: 'نسخ' }))
    await Promise.resolve()
    expect(denied.getByRole('button', { name: 'نسخ' })).toBeTruthy()
    denied.unmount()
