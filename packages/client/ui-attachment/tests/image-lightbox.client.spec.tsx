...
const labels = { dialog: 'معاينة الصورة الأصلية', close: 'إغلاق معاينة الصورة الأصلية' }
...
describe('ImageLightbox', () => {
  it('focuses its close control, closes by button and Escape, and restores focus', () => {
    ...
    opener.focus()
    const onClose = vi.fn()
    const view = render(<ImageLightbox src="blob:original" alt="الصورة الأصلية" labels={labels} onClose={onClose} />)
    const close = view.getByRole('button', { name: 'إغلاق معاينة الصورة الأصلية' })
  ...
  it('tolerates a focus owner it cannot restore (no active element at mount)', () => {
    ...
    try {
      const view = render(<ImageLightbox src="blob:original" alt="الصورة الأصلية" labels={labels} onClose={vi.fn()} />)
      view.unmount()
    } finally {
