...
describe('PermissionRow', () => {
  it('loads the descriptor, opens the menu, and selects a new default', async () => {
    ...
    mount(controller)
    const button = await screen.findByRole('button', { name: 'قراءة فقط' })
    ...
    expect(button.getAttribute('aria-expanded')).toBe('false')
    fireEvent.click(button)
    fireEvent.click(screen.getByRole('menuitem', { name: 'قراءة فقط' }))
    expect(mutate).not.toHaveBeenCalled()
    fireEvent.click(button)
    fireEvent.click(screen.getByRole('menuitem', { name: 'الكتابة في مساحات العمل' }))
    await screen.findByRole('button', { name: 'الكتابة في مساحات العمل' })
  ...
  it('requires explicit acknowledgement before saving full access', async () => {
    ...
    mount(controller)
    fireEvent.click(await screen.findByRole('button', { name: 'قراءة فقط' }))
    fireEvent.click(screen.getByRole('menuitem', { name: 'وصول كامل' }))
    expect(mutate).not.toHaveBeenCalled()
    fireEvent.click(screen.getByRole('button', { name: 'إلغاء' }))
    expect(screen.queryByRole('dialog', { name: 'تفعيل الوصول الكامل؟' })).toBeNull()
    fireEvent.click(screen.getByRole('button', { name: 'قراءة فقط' }))
    fireEvent.click(screen.getByRole('menuitem', { name: 'وصول كامل' }))
    const dialog = screen.getByRole('dialog', { name: 'تفعيل الوصول الكامل؟' })
    const enable = screen.getByRole('button', { name: 'تفعيل الوصول الكامل' })
  ...
  it('hides an unavailable namespace and disables a read-only provider', async () => {
    ...
    mount(readonly)
    expect((await screen.findByRole('button', { name: 'قراءة فقط' })).hasAttribute('disabled')).toBe(true)
  })
  ...
  it('shows loading and a contained write error', async () => {
    ...
    mount(controller)
    expect((await screen.findByRole('button', { name: 'جارٍ التحميل' })).hasAttribute('disabled')).toBe(true)
