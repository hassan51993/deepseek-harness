...
`head`،`tail` و `headTail` حسب بايت بينما غير محرف أو سطر حساب عدد: عملية فرعية إدارة طريق و HTTP متن كل هو بايت تدفق.`finish()` سوف في كل مقتطع موضع إصلاح قص لا كامل رمز نقطة، لذلك إرجاع نص أبدا سوف يحمل من مقتطع جذب دخول استبدال محرف، رمز نقطة أيضا أبدا سوف عبر يتم حذف في بين جزء إعادة بناء.
...
| `grep` | `ItemRetainer`،`head` | spill ملف استلام تجميع، تدريجي مطابقة معاينة مقتطع، قسم مجموعة، ترتيب |
...
- [spill سياسة](../../spill/spill-policy/README.ar.md)——تركيب `TextRetainer`، محيط التفاف spill ملف إشعار بناء محدود معاينة.
- [spill فرعي نظام](../../../docs/subsystems/spill.ar.md)——هذا مكتبة معاينة آلية الذي خدمة spill مفردات.
- [ملف بحث أداة](../../fs/tool-fs-search/README.ar.md)——لـ spill استلام تجميع كامل نتيجة `ItemRetainer` مستهلك.
...
- **نص إبقاء موجه إلى بايت**——`read` قسم صفحة انتظار سطر نافذة و محرف نافذة حاجة مفرد وحيد مصير؛ مقتطع ممكن إسقاط جزء UTF-8 حد بايت، بـ إبقاء إرجاع نص صالح.
