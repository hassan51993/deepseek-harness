...
describe('native file manager', () => {
  ...
  it('selects a translated WSL path in Explorer and never starts a Linux file manager', async () => {
    const run = vi.fn<PathOpenerRunner>(async () => ({ stdout: 'C:\\work\\تقرير إبلاغ.txt\r\n', stderr: '' }))
    const internals = { platform: 'linux' as const, env: { WSL_DISTRO_NAME: 'Ubuntu' }, run }
    expect(nativeFileManager(internals)).toBe('explorer')
    await revealNativePath('/mnt/c/work/تقرير إبلاغ.txt', signal(), internals)
    expect(run.mock.calls.map(([cmd, args]) => [cmd, args])).toEqual([
      ['wslpath', ['-w', '/mnt/c/work/تقرير إبلاغ.txt']], ['explorer.exe', ['/select,', 'file:///C:/work/%E6%8A%A5%E5%91%8A.txt']],
    ])
  })
...
it.each([
  ['C:\\my files\\تقرير إبلاغ,#%.txt', 'file:///C:/my%20files/%E6%8A%A5%E5%91%8A%2C%23%25.txt'],
