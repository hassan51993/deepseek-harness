...
هذا إشارة فقط مسؤول إشعار: استدعاء جهة يجب وصل دخول ذاتي ذات إنهاء آلية——يأخذ `d.signal` نقل إعطاء `fetch`، أو استماع `abort` و قتل ميت عملية فرعية. يجعل promise و timer تنافس سرعة، سوف في عملية فرعية أو طقم وصل حرف ما زال في تسرب تسرب وقت حينئذ يجعل استدعاء الأداة إتمام.
...
- [استدعاء الأداة مهلة سياسة](../../guard/timeout-policy/README.ar.md)——قوي صنع تنفيذ قد إعلان أداة مهلة مستهلك.
