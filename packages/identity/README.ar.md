---
description: "identity حزمة مجموعة: من بعيد قياس، عكس تغذية و DeepSeek مزود طلب مشترك مجهول اسم صلة ربط id، كل harness home واحد."
kind: "package-group"
---

# identity/ — مشترك هوية

[English](README.md) | العربية

## عام وصف

identity مجموعة لـ كل harness home توفير واحد مجهول اسم id، هذا تثبيت بعيد قياس، عكس تغذية و DeepSeek طلب سوف يأخذ هو مرفق إضافة إلى كل منها سجل فوق، لذلك مغادرة فتح نفس عدد home كل محتوى كل قدرة يتم تعرف آخر لـ قدوم ذاتي نفس طقم تثبيت، بينما بلا حاجة تعرف آخر مستخدم هوية. بلا حاجة إعداد أي شرق غرب:id سوف في هذه وظيفة لـ واحد أول مرة وقت التشغيل تلقائي ظهور، و في ملف يتم حذف قبل إبقاء مستقر. هذا مجموعة فقط لديه واحد حزمة؛ هذا صفحة صف خروج هذا مجموعة مجموعة صار، حزمة README مسؤول دقيق عقدة.

## دليل

- [حزمة](#packages)
- [متبادل صلة وثيقة](#related-documentation)
- [ملاحظة تطوير](#dev-note)

<a id="packages"></a>
## حزمة

| حزمة | مسؤولية |
|---|---|
| [`anonymous-user-id`](anonymous-user-id/README.ar.md) | يجعل كل harness home يملك واحد مجهول اسم id، بعيد قياس، عكس تغذية و DeepSeek طلب يأخذ هو مرفق إضافة إلى سجل فوق، جعل قدوم ذاتي نفس تثبيت سجل بلا حاجة تعرف آخر مستخدم يكفي يتم تمييز إقرار |

<a id="related-documentation"></a>
## متبادل صلة وثيقة

- [جلسة بعيد قياس فرعي نظام](../../docs/subsystems/session-telemetry.ar.md)——في توجيه خروج في يحمل هذا id بعيد قياس وظيفة.
- [dsh-llm-deepseek](../llm/llm-deepseek/README.ar.md)——في طلب في يحمل هذا id DeepSeek مزود.
- [dsh-command-feedback](../feedback/command-feedback/README.ar.md)——في تأكيد نص في نقطة اسم هذا مجهول اسم تثبيت عكس تغذية أمر.

<a id="dev-note"></a>
## ملاحظة تطوير

بلا.
