...
it('cancels installation through the UI, restores files, and offers the spec again', async () => {
  ...
  try {
    ...
    try {
      ...
      await dialog.getByText('الإصدار 1.0.0', { exact: true }).waitFor()
      await dialog.getByRole('button', { name: 'عرض تفاصيل التثبيت', exact: true }).click()
      await dialog.getByText('Waiting for package download', { exact: true }).waitFor()
      await dialog.getByRole('button', { name: 'إلغاء التثبيت', exact: true }).click()
      // The Host's confirmation returns the dialog to the spec and says so in a toast.
      await page.getByText('أُلغي التثبيت؛ الإضافة غير مفعّلة، وقد تبقى ملفات نُزِّلت', { exact: true }).waitFor()
      ...
      `)
      await dialog.getByRole('button', { name: 'تثبيت', exact: true }).click()
      await dialog.getByRole('button', { name: 'تفعيلها الآن', exact: true }).waitFor()
      await dialog.getByRole('button', { name: 'عرض تفاصيل التثبيت', exact: true }).click()
