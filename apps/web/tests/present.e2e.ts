...
const PROMPT = 'Use one run_code program to do the following in order. Call present for missing.txt and catch its error without creating that file. '
  ...
  + 'Call present for report.txt and شرح.txt. After present succeeds, deliberately throw the string "AFTER_PRESENT" (not an Error object) from that same run_code program. '
  + 'Do not retry the program or create any other files. Finish by mentioning `report.txt` and `شرح.txt` in inline code, and put PRESENT_DONE in a separate paragraph.'
...
`, { mode: 0o700 })
  ...
  it('opens current source files after edits and reload, and reports deletion without downloading', async () => {
    ...
    await writeFile(join(cwd, 'شرح.txt'), 'EDITED_NOTE\n')
    for (const reload of [false, true]) {
      ...
      for (const [name, content] of [['report.txt', 'EDITED_REPORT'], ['شرح.txt', 'EDITED_NOTE']] as const) {
