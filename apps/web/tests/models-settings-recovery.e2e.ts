...
describe('web e2e: repairs a stored provider after catalog drift', () => {
  ...
  it('shows the failed provider beside healthy providers and keeps both add actions usable', async () => {
    ...
    const dialog = page.getByRole('dialog', { name: 'الإعدادات' })
    expect(await dialog.getByRole('button', { name: 'تحرير openrouter', exact: true }).count()).toBe(1)
    ...
    expect(await dialog.getByRole('button', { name: 'إضافة مزوّد', exact: true }).isEnabled()).toBe(true)
    expect(await dialog.getByRole('button', { name: 'إضافة مزوّد مخصّص', exact: true }).isEnabled()).toBe(true)
    await compareOrRefreshGolden(EXPECTED, await captureStableAria(page, '[role="dialog"]', scaffold.workspaceCwd), webSnapshotMode())

    await dialog.getByRole('button', { name: 'إضافة مزوّد', exact: true }).click()
    ...
    await dialog.getByText('تم حفظ minimax-cn.', { exact: true }).waitFor()
  ...
  it('rejects an invalid edit without persisting and accepts removal of the obsolete model', async () => {
    const dialog = page.getByRole('dialog', { name: 'الإعدادات' })
    await dialog.getByRole('button', { name: 'تحرير openrouter', exact: true }).click()
    await dialog.getByText('إعدادات مخصّصة', { exact: true }).click()
    await dialog.getByLabel('العنوان الأساسي', { exact: true }).fill('https://gateway.example/v1')
    ...
    await dialog.getByRole('button', { name: 'حفظ', exact: true }).click()
    await dialog.getByText('تم حفظ openrouter.', { exact: true }).waitFor()
