...
export async function runInteractiveUpdates({ mainWindow, server, fixture, checkMenu, control, root }) {
  ...
  fixture.updater.quitAndInstall = (...args) => {
    ...
    void dialog.showMessageBox(panel, { type: 'info', title: 'محلي تحقق استلام انتهاء',
      message: 'اكتمل تحت تحميل، تحقق، تثبيت تأكيد و مهمة استلام ذيل.',
  ...
  try {
    await panel.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(`<!doctype html><html lang="ar-SA">
      ...
      <p>تحت تحميل سوف إبقاء دخول درجة، توفير أنت عرض. عودة إلى هذا داخل اختيار<strong>وضع سطر حالي تحت تحميل</strong>، عندئذ سوف دخول تحقق و تثبيت تأكيد.</p>
      <p>استخدام<strong>مهمة حالة</strong>قائمة مفرد إضافة ترتيب طابور مهمة، عرض تثبيت قبل مهمة تحذير إبلاغ. فشل نمط يجب في تحت تحميل قبل اختيار.</p>
