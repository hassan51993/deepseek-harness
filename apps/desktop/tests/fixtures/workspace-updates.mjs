...
async function qualify() {
  ...
  try {
    ...
    const acknowledgeNotice = `[...document.querySelectorAll('button')].find(button => button.textContent.trim() === 'متابعة')`
    ...
    await documentReady(mandatory, `document.getElementById('title')?.textContent === 'حاجة تحديث'`)
    ...
    assert.equal(await mainWindow.webContents.executeJavaScript("document.body.innerText.includes('إعادة اتصال في')"), false)
