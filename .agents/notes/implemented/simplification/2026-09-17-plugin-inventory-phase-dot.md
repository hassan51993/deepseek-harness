...
The Settings plugin list marks every Loader entry with an enablement tag (`قد تفعيل`, `قد توقف استخدام`, `شرط تفعيل`, `مسبق ضبط في تفعيل`, `بدء فشل`). It also drew a colored root-fiber status dot on enabled rows whose fiber was live. For the common `active` phase the dot and the tag stated the same thing twice on one collapsed card, while `pending`, `loading` and `unloading` — the phases the tag has no words for — were where the dot carried its own information.
...
Row enablement, tag tones, grouping, search and list behavior are unchanged, as are the Loader lifecycle and persistence. The expanded card still lists the phase under تشغيل حالة / Status, so the runtime phase stays readable without the collapsed dot.
...
- Rows mid-transition keep their dot, its animation and its `role="img"` name, so the accessible name still says `انتظار اعتماد`, `تحميل في` or `إزالة في`.
- Known gap, pre-existing and unchanged: the dot is gated on `row.enabled`, so a row a user just disabled while its fiber is still `unloading` shows `قد توقف استخدام` without a dot.

## Testing

The package spec pins the rule in both scopes: `loading` and `pending` (global) and `unloading` (preset) keep a dot, while `active`, `failed` and a row without a live fiber render none. `apps/web/tests/settings-chrome.e2e.ts` asserts the assembled global group contains enabled rows and no `role="img"` element named `تشغيل في`.
