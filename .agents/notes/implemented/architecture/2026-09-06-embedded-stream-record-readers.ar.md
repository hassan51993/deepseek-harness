...
`packages/llm/llm/src/assistant-stream.ts` في تراكم إضافة جهاز و `expandAssistantStream` خارج تصدير سجل درجة قراءة جهاز:
...
**حسب إدخال عدد مجموعة تسجيل ذاكرة تحويل `expandAssistantStream`.** توسيع الكل تدفق فقط يحتاج بضعة عشرة مللي ثانية، لكن إبقاء توسيع نتيجة في حدث دورة الحياة داخل نحو زهرة استهلاك ضيق تجميع تدفق عشرة ضعف داخل تخزين——هذا هو هذا تغيير إزالة لحظة وقت قسم إعداد دائم دائم إصدار. قراءة جهاز تماما إزالة حذف مقابل إبقاء توسيع يحتاج طلب.
