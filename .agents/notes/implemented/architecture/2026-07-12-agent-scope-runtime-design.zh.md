...
[agent(ذكي جسم) أثر مجال اتفاق](2026-07-08-agent-scope-contexts.zh.md) مقابل مساهمة من بينما قول جدا بسيط مفرد: عبر `agent.ctx` تسجيل، تحليل خروج واحد عام إضافة مفرد agent عرض، فقط في setup إتمام بعد إصدار، و إبقاء أثر مجال مباشر إلى عمل إيقاف. وقت التشغيل يجب في تنسيق عمل صيغة إضافة إطار هيكل، مختلف خطوة إنشاء، يمكن إعادة دخول مستمع، حفظ دائم جلسة إيداع و worker أو عملية لذا عائق انتظار مشهد تحت صيانة هذا نسخة اتفاق.
...
[7 شهر 8 يوم Agent Note](2026-07-08-agent-scope-contexts.zh.md) ما زال هو مساهمة من اتفاق. مستقل [subagent تركيب تحكم Agent Note](../feature/2026-07-12-subagent-persona-tool-filter-and-depth.zh.md) يملك `persona`،`toolFilter` و `maxDepth`؛ هذا نص فقط نقاش نقاش هو جمع setup مثل أي دمج دخول دورة الحياة.
...
Agent في اثنان عدد سجل التسجيل و إنشاء مستمع كل إتمام قبل أبدا قيادة. مستمع يمكن رفض أو dispose واحد كل من؛ أمر خدمة إبقاء أثر مجال و جلسة، انتظار توزيع تسوية بعد مجددا متابعة تفكيك حذف. كل قد بدء إنشاء إعلان إبلاغ في تراجع خلال كل لديه مطابقة إلغاء تدمير إعلان إبلاغ.[يمكن انتظار إنشاء قرار](2026-09-09-awaited-agent-creation.zh.md) يملك مختلف خطوة ابتدائي تحويل جهاز وقت ترتيب.
...
[فقط موجه إلى تلقائي تحويل ACP جسر وصل طبقة](../simplification/2026-07-23-acp-automation-only-protocol.zh.md) مباشر سوف واحد إجراء في نص التوجيه و ذلك مراقبة إلى مستخدم رسالة جولة صلة ربط. هو لا من سجل ماء موضع خط مسح، أيضا لا استخدام جلسة حالة بصفة ثاني عدد ضبط و مسبق قول آلة.
...
[7 شهر 8 يوم Agent Note](2026-07-08-agent-scope-contexts.zh.md#alternatives-considered) يملك عام مسطح مستو أثر مجال اتفاق بديل خطة. هذا موضع بديل خطة صلة ملاحظة تنفيذ شكل.
...
[أمان و إذن غير هدف](2026-07-08-agent-scope-contexts.zh.md#security-and-authority-are-non-goals) ما زال هو أصل هذا صفة. هذه آلية إثبات تسجيل تركيب، إصدار و توليد أمر مدة كل حق؛ هو جمع لا إثبات عزل أو أب إلى فرعي غير رفع حق.
