...
**إعطاء يمكن تحويل إرسال حدث تحميل حمل إسقاط دالة**(`{ حدث اسم, إسقاط, zod }` تحويل إرسال جدول). قدرة واحد رفع تغطية `models-changed` fan-in و workspace view إرسال توليد؛ بديل قيمة هو إسقاط منطق و تحميل حمل نوع يد عمل مقابل متساو، عودة إلى طريقة جانب للتو إزالة إطفاء في قلب جدول شكل.
