...
Web وصول هو واحد واحد انتظار قدرة seam، التزام دوران[قدرة seam Agent Note](2026-06-13-capability-seams.zh.md):
...
كل واحد طبقة في ذاتي ذات حد موضع ثابت:`dsh-web` في تسجيل/اختيار/قطع قطع/abort اتفاق و `WebError` رمز؛ كل مزود أساس في تسجيل صنع fixture(اختبار قبل وضع بيانات) طلب/استجابة خريطة (Perplexity fixture يتضمن صاف URL مرجع، بـ إبقاء اختياري source حقل صدق فعلي صفة) ، إضافة فوق كل حقيقي مزود ذاتي قفز مرور حمل مفتاح خطر دخان اختبار؛`web-fetch-http` في حقيقي محلي HTTP سلوك؛`dsh-tool-web` في عبر حقيقي أداة سجل التسجيل تفعيل قيادة تسجيل، بنية تحويل تنفيذ خطأ و نتيجة صيغة تحويل. واحد حقيقي Loader خطر دخان اختبار حراسة حماية اثنان نوع توجيه خروج شكل حالة ([أمر لذا تكرار قرص (postmortem) 0001](../../../../docs/postmortem/0001-acp-default-export-drops-inject.zh.md)):`dsh-web` هو افتراضي توجيه خروج خدمة، بينما مزود و `tool-web` هو نطاق الأسماء إضافة، خطأ إضافة `export default` سوف فقد فقد `inject`.
