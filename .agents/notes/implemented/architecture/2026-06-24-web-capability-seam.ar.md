...
اختياري signal هو تنفيذ تحكم، بينما غير عمل خدمة إدخال:`tool-web` مباشر نقل تمرير `exec.signal`، جعل جولة إلغاء، أداة مهلة و agent dispose(مورد تحرير) قدرة وصول مزود شبكة طلب، تدفق قراءة جهاز و عال فتح إلغاء حل رمز.seam لا نقل تمرير `ToolExecution`——لا فإن `dsh-web` حينئذ يلزم اعتماد `dsh-tools`.
...
مزود متاح صفة و قدرة اختيار هو اثنان عدد مستقل عام فكرة، لكن كل إبقاء الأكثر صغير تحويل. مزود فقط تقرير إبلاغ هذا أداة جسم تنفيذ هل متاح، عبر نزيه قيمة محلي فحص (مثل سند إثبات هل وجود، طرف نقطة إعداد هل يمكن تحليل). مزود `available()` منع توقف إرسال بدء شبكة استدعاء.
...
HTTP حالة رمز هو قد نيل أخذ مورد حالة واحد جزء، لا تلقائي بنية صار أداة فشل. عبر شبكة نجاح نيل أخذ إلى `404` أو `500` استجابة وقت، سوف إرجاع حمل لديه حالة رمز و محدود حل رمز متن (عند محتوى نوع تلقي دعم حمل وقت) `WebFetchResult`.`WebError` لأجل لا يمكن أمان نيل أخذ أو يمثل مورد فشل: بلا فاعلية أو يتم منع قطع URL، إعادة تحديد نحو سياسة مخالفة قاعدة، مهلة،abort، استجابة مرور كبير، لا دعم حمل محتوى نوع، مزود فشل أو شبكة فشل.
...
مزود مسؤول أمان مورد نيل أخذ:URL تحقق،HTTP نقل، إعادة تحديد نحو سياسة، مهلة،abort نقل بث، بايت حد أعلى، محرف تجميع حل رمز، محتوى نوع تصنيف و اثنان دخول صنع رفض.`dsh-tool-web` مسؤول عرض:HTML تحويل Markdown،HTML تحويل صاف نص، موجه إلى نموذج مقتطع صيغة تحويل، و لم قدوم ملخص.
...
- Abort إشارة نقل بث إلى شبكة نيل أخذ و عال فتح إلغاء حل رمز.
- فقط تلقائي تتبع مع نفس مصدر إعادة تحديد نحو؛ كل تتبع مع قفز تحويل كل سوف إعادة تحليل عام عنوان، و يأخذ ذاتي ذات اتصال ثابت إلى تحليل نتيجة. عبر مصدر إعادة تحديد نحو بـ `WEB_REDIRECT_BLOCKED` فشل، اشتراط مرة جديد استدعاء الأداة و جديد عام عنوان تحقق.(Claude Code WebFetch استخدام نفس مثال نموذج——هو لا تلقائي تتبع مع عبر رئيسي آلة إعادة تحديد نحو، بينما هو سوف إعادة تحديد نحو هدف إرجاع إعطاء نموذج بـ إرسال بدء جديد استدعاء.)
- طلب يحمل صريح منتج User-Agent، بينما غير ساكن صامت زائف تركيب متصفح.

فقط يلزم DNS كامل تحليل نتيجة في وجود مهمة واحد غير عام عنوان، مزود حينئذ سوف رفض كامل نتيجة، بينما لا هو ساكن صامت مرور ترشيح لا أمان عضو. هذا fail-closed قاعدة يمكن منع توقف اتصال عنوان عائلة اختيار أو رجوع لمس و لم ممتلئ كاف عام شبكة سياسة عنوان.
...
كل واحد طبقة في ذاتي ذات حد موضع ثابت:`dsh-web` في تسجيل/اختيار/مقتطع/abort اتفاق و `WebError` رمز؛ كل مزود أساس في تسجيل صنع fixture(اختبار قبل وضع بيانات) طلب/استجابة خريطة (Perplexity fixture يتضمن صاف URL مرجع، بـ إبقاء اختياري source حقل صدق فعلي صفة) ، إضافة فوق كل حقيقي مزود ذاتي قفز مرور حمل مفتاح خطر دخان اختبار؛`web-fetch-http` في حقيقي محلي HTTP سلوك؛`dsh-tool-web` في عبر حقيقي أداة سجل التسجيل تفعيل قيادة تسجيل، بنية تحويل تنفيذ خطأ و نتيجة صيغة تحويل. واحد حقيقي Loader خطر دخان اختبار حراسة حماية اثنان نوع تصدير شكل حالة ([أمر لذا تكرار قرص (postmortem) 0001](../../../../docs/postmortem/0001-acp-default-export-drops-inject.ar.md)):`dsh-web` هو افتراضي تصدير خدمة، بينما مزود و `tool-web` هو نطاق الأسماء إضافة، خطأ إضافة `export default` سوف فقد فقد `inject`.
...
قد تسليم preset لا اعتماد هذا واحد خطة. عام عنوان تحقق سوف منع قطع SSRF هدف عنوان، بينما تدريجي مرة تأكيد سوف ضرب قطع عادي تصفح، لكن لا يستطيع يمكن اعتماد تحكم عام بيانات خروج محطة: نموذج يمكن عبر قد تركيب shell أداة وصول نفس عام شبكة. اشتراط مخصص باب تأكيد خطوة نشر يمكن إضافة `tools/pre-execute` سياسة أو منع استخدام `web_fetch`.
