...
Cordis هو SDK قاع طبقة إضافة إطار هيكل.Cordis **سياق**هو إضافة استخدام قدوم وصول خدمة و تسجيل فاعلية نتيجة كائن، فاعلية نتيجة تنظيف تتبع مع هذا سياق.[Cordis دخول باب](../../../../docs/cordis-primer.zh.md) مقابل هذا إطار هيكل لديه أكثر تفصيل دقيق شرح.
...
إعداد طقم[وقت التشغيل تصميم Agent Note](2026-07-12-agent-scope-runtime-design.zh.md) توضيح وصف تنفيذ و صحيح تأكيد صفة دفع إدارة.[صريح وقت التشغيل هوية Agent Note](2026-08-31-explicit-agent-runtime-identity.zh.md) شرح دورة الحياة، حدث و نقل واجهة لـ أي صريح نقل تمرير Agent هوية، بينما لا عبر Context كشف هذا هوية.[subagent تركيب تحكم Agent Note](../feature/2026-07-12-subagent-persona-tool-filter-and-depth.zh.md) مسؤول مستقل `persona`،`toolFilter` و `maxDepth` وظيفة.
...
بـ `{ global: true }` تسجيل مستمع متعمد التفاف مرور سياق تلقي جمهور مرور ترشيح، لكن ذلك تنظيف ما زال تتبع مع تسجيل سياق. سجل التسجيل عضو تغيير إشعار إبقاء لا مرور ترشيح، لأن هو جمع وصف هو مشترك سجل التسجيل حالة بينما غير بعض عدد agent عملية. تفصيل كل حدث مشاركة اعتبار هو كل[فرعي نظام صفحة](../../../../docs/subsystems/core.zh.md) فوق توليد `cordis-surface` منطقة كتلة تجميع دمج——كل حدث أثر مجال في ذلك الذي تابع صفحة فوق (`agent/*` و `agent-loop/*` في core.md هذا صفحة).
