...
English | [العربية](2026-07-31-one-route-to-add-a-workspace.ar.md)
...
Adding a Workspace has one route: pick a host directory through the composed directory flow, new or existing. `menu.addWorkspace` ("إضافة مساحة العمل…" / "Add workspace…") is the entry; the create-by-name dialog and its `create.*` / `menu.createWorkspace` / `workspace.new` strings are gone. The label names the outcome, not the mechanism, because it is now the only door to that outcome — a user looking for "جديد بناء" must find it.
