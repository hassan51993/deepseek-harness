...
The model side remains capped at `searchMaxResults`: the seam caps each provider result, the multi-query consumer caps a combined list, and the `truncated` flag and its `مصدر قائمة قد قطع قطع` indicator stay. The card draws the final tool source list in full and scrollable, instead of collapsing its middle.
