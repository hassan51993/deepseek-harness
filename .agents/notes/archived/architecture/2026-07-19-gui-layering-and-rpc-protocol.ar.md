...
> قسم عمل خط: هذا مقالة = قسم طبقة نموذج + عبر طريق غير متصل RPC بروتوكول؛ بروتوكول Web تنفيذ من HTTP فوق سطر إضافة [WebSocket تحت سطر تحميل جسم](2026-08-04-websocket-downlink-carrier.ar.md) مجموعة صار، متصفح كائن طبقة رؤية [Web عميل هيكل بنية قلم تسجيل](2026-07-19-gui-web-client-architecture.ar.md).
...
- `packages/client/*`: حزمة فقط توفير Client جانب قدرة، كل حزمة مفرد حافة لا خلط. هذا داخل إقامة ثلاثة صنف حزمة (اثنان بند محور عودة [client إضافة تركيب تحميل قلم تسجيل](2026-07-23-client-plugin-loading-model.ar.md) كل):
...
- `apps/` بصفة مقابل خارج توجيه خروج تطبيق مدخل، يمكن من Client / Host خلط دمج تجميع.
    ...
    - `apps/cli`(`@deepseek-ai/dsh`) توزيع أمر:`dsh web` = Host + webserver + بناء خروج `dsh-web-frontend` dist؛`dsh --profile headless` = [مباشر استخدام نواة قلب Agent/Session مدخل](2026-08-09-headless-direct-core-entry-point.ar.md) ، لا يحتوي Host،HTTP أو متصفح طبقة.
...
- client جانب عبر حزمة import إضافة حزمة واحد قاعدة مشي `/client` فرعي مسار، كما إضافة حزمة بين فقط حد نوع import——عبر إضافة قيمة import في tsdown صاف درجة بوابة موضع أي بناء خطأ (قيمة طبقة وجه تنسيق عمل مشي cordis خدمة؛ حافة قاعدة عودة [client إضافة تركيب تحميل قلم تسجيل](2026-07-23-client-plugin-loading-model.ar.md) كل).

TypeScript بـ solution أصل مرجع**اثنان عدد تجمع دمج program** فحص (`tsconfig.json` = solution؛`tsconfig.host.json` = host جانب + اختبار، ترتيب حذف `packages/client`؛`tsconfig.client.json` = client كل حزمة و ذلك اختبار): اثنان جانب في نفسه مفتاح (`sessions`،`loader`) تحت بـ مختلف خدمة دمج cordis `Context` واجهة، مفرد واحد program سوف معا يرى اثنان نسخة إعلان دمج بينما تقرير اندفاع مفاجئ. مشترك ورقة فرعي حزمة (session/llm/tools/apiproxy انتظار) فقط بناء مرة، من اثنان عدد program مشترك نفس مرجع ([توسيع اندفاع](../process/2026-07-22-tsconfig-solution-root-two-aggregates.ar.md)).
...
| `WebApiClient` | dsh-client-connection | `globalThis.fetch` فوق سطر + كل منطق تدفق واحد بند نفس مصدر WebSocket تحت سطر | متصفح عميل؛ شيء إدارة حد رؤية [WebSocket تحت سطر تحميل جسم](2026-08-04-websocket-downlink-carrier.ar.md) |
