...
- `read-only` و `workspace-write` حل تفسير native Landlock launcher بروتوكول، و في VFS لقطة بوابة فتحة تنفيذ تدريجي عملية تخويل؛`danger-full-access` إبقاء مباشر عملية مسار.[Watcher و confinement قرار](2026-08-23-webworker-vfs-watch-and-landlock.ar.md) يملك هذا تنفيذ عالم حد في `full` أكثر ضيق يحتوي معنى.
