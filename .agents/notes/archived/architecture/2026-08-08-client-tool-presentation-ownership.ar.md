...
Conversation بيانات تجميع التزام دوران لاحق [Conversation عمل خدمة عقدة قرار](2026-08-09-client-conversation-node-assembly.ar.md).`ui-conversation` أداة Definition من جلسة حدث إعداد مقابل root call/result، يأخذ Code Dispatch edge fold صار تمرير عودة `ToolCallBlock.subCalls`، و توليد واحد مستقر `tool-call` Chat Node؛ هذا داخل بيانات مسؤولية فقط معالجة رسمي جهة أداة identity و توسيع اندفاع، لا حل تفسير أداة جسم أداة اسم عرض.
