...
`readUtf16` in `packages/host/directory-picker-native/src/win32-dialog-bindings.ts` translated the `IFileOpenDialog` result buffer by scanning for a zero byte with `bytes[end] !== 0`. UTF-16LE encodes NUL as two zero bytes, so any BMP code unit whose low byte is zero — U+XX00, such as فتح (U+5F00) — ended the scan early. Selecting a folder like `C:\Users\XIAOPAN\Desktop\أمان بارز تطوير` returned `C:\Users\XIAOPAN\Desktop\أمان بارز`, and the workspace-creation call failed with `workspace-invalid-path ... ENOENT`.

## Decision

The scan ends only when both bytes of a code unit are zero, still advancing two bytes at a time over the same 32KiB `koffi.view` buffer. A regression test drives `readUtf16` through the existing fake koffi COM world with a path containing أمان بارز تطوير (U+5F00), so the termination rule is proven without a real Windows host.
...
- Real-dialog rendering and selection remain a manual Windows check; this change's regression test exercises only the byte-to-string translation against the fake COM world. The fixture path is synthetic (`C:\fixture\أمان بارز تطوير`) so no real user path appears in the repository.
